...
class TestParsePolicyDocument:
    ...
    def test_multiple_actions_as_list(self):
        doc = {"Statement": [{"Effect": "Allow", "Action": ["s3:Get*", "s3:List*"], "Resource": "*"}]}
        stmts = parse_policy_document(doc)
    ...
    def test_string_json_input(self):
        stmts = parse_policy_document(json.dumps({"Statement": [{"Effect": "Allow", "Action": "*", "Resource": "*"}]}))
        assert len(stmts) == 1
...
class TestValidatePolicyDocument:
    def test_valid_policy(self):
        assert validate_policy_document({"Statement": [{"Effect": "Allow", "Action": "s3:*", "Resource": "*"}]}) is None
    ...
    def test_invalid_effect(self):
        assert (
            validate_policy_document({"Statement": [{"Effect": "Maybe", "Action": "s3:*", "Resource": "*"}]})
            is not None
        )

    def test_missing_action(self):
        assert validate_policy_document({"Statement": [{"Effect": "Allow", "Resource": "*"}]}) is not None

    def test_missing_resource(self):
        assert validate_policy_document({"Statement": [{"Effect": "Allow", "Action": "s3:*"}]}) is not None

    def test_both_action_and_not_action(self):
        assert (
            validate_policy_document(
                {"Statement": [{"Effect": "Allow", "Action": "s3:*", "NotAction": "iam:*", "Resource": "*"}]}
            )
            is not None
        )
...
class TestEvaluate:
    def test_explicit_allow(self):
        stmts = parse_policy_document({"Statement": [{"Effect": "Allow", "Action": "s3:*", "Resource": "*"}]})
    ...
    def test_not_action_excludes_specified(self):
        stmts = parse_policy_document({"Statement": [{"Effect": "Allow", "NotAction": "iam:*", "Resource": "*"}]})
        assert evaluate(_ctx(action="s3:PutObject"), [stmts]).decision == "Allow"
        assert evaluate(_ctx(action="iam:CreateUser"), [stmts]).decision == "ImplicitDeny"

    def test_not_resource_excludes_specified(self):
        stmts = parse_policy_document(
            {"Statement": [{"Effect": "Allow", "Action": "s3:*", "NotResource": "arn:aws:s3:::secret-bucket/*"}]}
        )
    ...
    def test_multiple_policies_union(self):
        ...
        p1 = parse_policy_document({"Statement": [{"Effect": "Allow", "Action": "s3:Get*", "Resource": "*"}]})
        p2 = parse_policy_document({"Statement": [{"Effect": "Allow", "Action": "s3:Put*", "Resource": "*"}]})
    ...
    def test_admin_policy_allows_everything(self):
        stmts = parse_policy_document({"Statement": [{"Effect": "Allow", "Action": "*", "Resource": "*"}]})
    ...
    def test_deny_in_one_policy_overrides_allow_in_another(self):
        ...
        allow = parse_policy_document({"Statement": [{"Effect": "Allow", "Action": "*", "Resource": "*"}]})
        deny = parse_policy_document({"Statement": [{"Effect": "Deny", "Action": "s3:Delete*", "Resource": "*"}]})
                ...
                "Statement": [
                    {
                        ...
                        "Action": "*",
                        "Resource": "*",
                        "Condition": {"ArnLike": {"aws:PrincipalArn": "arn:aws:iam::000000000000:user/*"}},
                    }
                ]
            }
        )
                ...
                "Statement": [
                    {
                        ...
                        "Action": "*",
                        "Resource": "*",
                        "Condition": {"StringEqualsIfExists": {"aws:RequestedRegion": "eu-west-1"}},
                    }
                ]
            }
        )
                ...
                "Statement": [
                    {
                        ...
                        "Action": "*",
                        "Resource": "*",
                        "Condition": {"ForAllValues:StringEquals": {"aws:TagKeys": ["env", "team", "project"]}},
                    }
                ]
            }
        )
                ...
                "Statement": [
                    {
                        ...
                        "Action": "*",
                        "Resource": "*",
                        "Condition": {"ForAnyValue:StringEquals": {"aws:TagKeys": ["env", "team"]}},
                    }
                ]
            }
        )
...
class TestResourceAccountCondition:
    ...
    def test_resource_arn_account_field_wins(self):
        ...
        ctx = _ctx(action="sqs:SendMessage", resource="arn:aws:sqs:us-east-1:222222222222:queue")
        assert evaluate(ctx, [stmts]).decision == "Allow"
        ctx_own = _ctx(action="sqs:SendMessage", resource="arn:aws:sqs:us-east-1:000000000000:queue")
...
class TestTrustPolicy:
    ...
    def test_account_id_shorthand(self):
        ...
        trust = {"Statement": [{"Effect": "Allow", "Principal": {"AWS": "123456789012"}, "Action": "sts:AssumeRole"}]}
        assert evaluate_trust_policy(trust, "arn:aws:iam::123456789012:user/alice")
        assert not evaluate_trust_policy(trust, "arn:aws:iam::999999999999:user/bob")
    ...
    def test_wrong_action_not_matched(self):
        trust = {"Statement": [{"Effect": "Allow", "Principal": "*", "Action": "s3:GetObject"}]}
    ...
    def test_string_json_input(self):
        trust_str = json.dumps({"Statement": [{"Effect": "Allow", "Principal": "*", "Action": "sts:AssumeRole"}]})
...
class TestEnforce:
    ...
    def test_user_with_matching_policy_allowed(self):
        ...
        iam_svc._user_inline_policies["enforce-user"] = {
            "p": json.dumps({"Statement": [{"Effect": "Allow", "Action": "s3:*", "Resource": "*"}]})
        }
        try:
    ...
    def test_user_inherits_group_inline_policy(self):
        ...
        iam_svc._group_inline_policies["dev-team"] = {
            "s3-access": json.dumps({"Statement": [{"Effect": "Allow", "Action": "s3:*", "Resource": "*"}]})
        }
        try:
    ...
    def test_user_inherits_user_inline_policy(self):
        ...
        iam_svc._user_inline_policies["inline-user"] = {
            "my-policy": json.dumps({"Statement": [{"Effect": "Allow", "Action": "dynamodb:*", "Resource": "*"}]})
        }
        try:
...
class TestSeededAwsManagedPolicies:
    ...
    def test_lambda_insights_log_writes_are_scoped_to_its_log_group(self):
        ...
        stmts = parse_policy_document(
            _resolve_managed_policy_document(
    ...
    def test_autocreate_applies_to_enforcement(self, monkeypatch):
        ...
        assert doc and evaluate(_ctx(action="sqs:SendMessage"), [parse_policy_document(doc)]).decision == "Allow"
...
class TestResourceArn:
    def test_s3_bucket(self):
        ...
        assert (
            extract_resource_arn("s3", "PUT", "/mybucket", {}, b"", {}, "us-east-1", "123") == "arn:aws:s3:::mybucket"
        )
    ...
    def test_s3_object(self):
        ...
        assert (
            extract_resource_arn("s3", "GET", "/mybucket/path/to/key", {}, b"", {}, "us-east-1", "123")
    ...
    def test_dynamodb_table(self):
        ...
        assert (
            extract_resource_arn("dynamodb", "POST", "/", {}, body, {}, "us-east-1", "123")
    ...
    def test_dynamodb_index(self):
        ...
        body = json.dumps({"TableName": "users", "IndexName": "email-index"}).encode()
        assert (
            extract_resource_arn("dynamodb", "POST", "/", {}, body, {}, "us-east-1", "123")
    ...
    def test_eventbridge_put_events_defaults_to_default_bus(self):
        ...
        assert (
            extract_resource_arn("events", "POST", "/", {}, body, {}, "us-east-1", "123")
    ...
    def test_lambda_function(self):
        ...
        assert (
            extract_resource_arn("lambda", "GET", "/2015-03-31/functions/my-func", {}, b"", {}, "us-east-1", "123")
    ...
    def test_lambda_invoke(self):
        ...
        assert (
            extract_resource_arn(
                "lambda", "POST", "/2015-03-31/functions/my-func/invocations", {}, b"", {}, "us-east-1", "123"
            )
            == "arn:aws:lambda:us-east-1:123:function:my-func"
        )
    ...
    def test_sqs_create_queue(self):
        ...
        assert (
            extract_resource_arn("sqs", "POST", "/", {}, b"", {"QueueName": ["my-queue"]}, "us-east-1", "123")
            == "arn:aws:sqs:us-east-1:123:my-queue"
        )
    ...
    def test_kms_key_id(self):
        ...
        assert (
            extract_resource_arn("kms", "POST", "/", {}, body, {}, "us-east-1", "123")
            == "arn:aws:kms:us-east-1:123:key/abc-123"
        )
    ...
    def test_kms_alias(self):
        ...
        assert (
            extract_resource_arn("kms", "POST", "/", {}, body, {}, "us-east-1", "123")
            == "arn:aws:kms:us-east-1:123:alias/my-key"
        )
    ...
    def test_secretsmanager(self):
        ...
        assert (
            extract_resource_arn("secretsmanager", "POST", "/", {}, body, {}, "us-east-1", "123")
    ...
    def test_iam_role(self):
        ...
        assert (
            extract_resource_arn("iam", "POST", "/", {}, b"", {"RoleName": ["my-role"]}, "us-east-1", "123")
    ...
    def test_iam_user(self):
        ...
        assert (
            extract_resource_arn("iam", "POST", "/", {}, b"", {"UserName": ["alice"]}, "us-east-1", "123")
    ...
    def test_kinesis_stream(self):
        ...
        assert (
            extract_resource_arn("kinesis", "POST", "/", {}, body, {}, "us-east-1", "123")
    ...
    def test_logs_group(self):
        ...
        assert (
            extract_resource_arn("logs", "POST", "/", {}, body, {}, "us-east-1", "123")
            == "arn:aws:logs:us-east-1:123:log-group:/app/logs"
        )
    ...
    def test_states_machine(self):
        ...
        assert (
            extract_resource_arn("states", "POST", "/", {}, body, {}, "us-east-1", "123")
    ...
    def test_ecs_cluster(self):
        ...
        assert (
            extract_resource_arn("ecs", "POST", "/", {}, body, {}, "us-east-1", "123")
    ...
    def test_eks_cluster_from_path(self):
        ...
        assert (
            extract_resource_arn("eks", "GET", "/clusters/prod", {}, b"", {}, "us-east-1", "123")
    ...
    def test_rds_instance(self):
        ...
        assert (
            extract_resource_arn("rds", "POST", "/", {}, b"", {"DBInstanceIdentifier": ["mydb"]}, "us-east-1", "123")
    ...
    def test_cloudformation_stack(self):
        ...
        assert (
            extract_resource_arn(
                "cloudformation", "POST", "/", {}, b"", {"StackName": ["my-stack"]}, "us-east-1", "123"
            )
    ...
    def test_ssm_parameter(self):
        ...
        assert (
            extract_resource_arn("ssm", "POST", "/", {}, b"", {"Name": ["/app/config"]}, "us-east-1", "123")
            == "arn:aws:ssm:us-east-1:123:parameter/app/config"
        )
    ...
    def test_elb_passthrough_arn(self):
        ...
        assert (
            extract_resource_arn(
                "elasticloadbalancing", "POST", "/", {}, b"", {"LoadBalancerArn": [lb_arn]}, "us-east-1", "123"
            )
            == lb_arn
        )
    ...
    def test_cloudfront_distribution(self):
        ...
        assert (
            extract_resource_arn(
                "cloudfront", "GET", "/2020-05-31/distribution/E123ABC", {}, b"", {}, "us-east-1", "123"
            )
            == "arn:aws:cloudfront::123:distribution/E123ABC"
        )
    ...
    def test_route53_hostedzone(self):
        ...
        assert (
            extract_resource_arn("route53", "GET", "/2013-04-01/hostedzone/Z123", {}, b"", {}, "us-east-1", "123")
    ...
    def test_cognito_userpool(self):
        ...
        assert (
            extract_resource_arn("cognito-idp", "POST", "/", {}, body, {}, "us-east-1", "123")
    ...
    def test_scheduler(self):
        ...
        assert (
            extract_resource_arn("scheduler", "GET", "/schedules/my-sched", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:scheduler:us-east-1:123:schedule/default/my-sched"
        )
    ...
    def test_pipes(self):
        ...
        assert (
            extract_resource_arn("pipes", "GET", "/v1/pipes/my-pipe", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:pipes:us-east-1:123:pipe/my-pipe"
        )
    ...
    def test_mq_broker(self):
        ...
        assert "my-broker" in extract_resource_arn(
            "mq", "GET", "/v1/brokers/my-broker", {}, b"", {}, "us-east-1", "123"
        )

    def test_kafka_cluster(self):
        from ministack.core.iam_actions import extract_resource_arn

        assert "kafka" in extract_resource_arn(
            "kafka", "GET", "/v1/clusters/my-cluster", {}, b"", {}, "us-east-1", "123"
        )
    ...
    def test_dsql_cluster(self):
        ...
        assert (
            extract_resource_arn("dsql", "GET", "/clusters/cl-123", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:dsql:us-east-1:123:cluster/cl-123"
        )
    ...
    def test_efs_filesystem(self):
        ...
        assert (
            extract_resource_arn(
                "elasticfilesystem", "GET", "/2015-02-01/file-systems/fs-123", {}, b"", {}, "us-east-1", "123"
            )
            == "arn:aws:elasticfilesystem:us-east-1:123:file-system/fs-123"
        )
    ...
    def test_cloudtrail(self):
        ...
        assert (
            extract_resource_arn("cloudtrail", "POST", "/", {}, body, {}, "us-east-1", "123")
            == "arn:aws:cloudtrail:us-east-1:123:trail/my-trail"
        )
    ...
    def test_appconfig_application(self):
        ...
        assert (
            extract_resource_arn("appconfig", "GET", "/applications/app-123", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:appconfig:us-east-1:123:application/app-123"
        )
    ...
    def test_appconfig_environment(self):
        ...
        assert (
            extract_resource_arn(
                "appconfig", "GET", "/applications/app-1/environments/env-1", {}, b"", {}, "us-east-1", "123"
            )
            == "arn:aws:appconfig:us-east-1:123:application/app-1/environment/env-1"
        )
    ...
    def test_resource_groups(self):
        ...
        assert (
            extract_resource_arn("resource-groups", "GET", "/groups/my-group", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:resource-groups:us-east-1:123:group/my-group"
        )
    ...
    def test_rds_data_passthrough(self):
        ...
        assert (
            extract_resource_arn("rds-data", "POST", "/", {}, body, {}, "us-east-1", "123")
    ...
    def test_s3tables(self):
        ...
        assert (
            extract_resource_arn("s3tables", "GET", "/buckets/my-tb", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:s3tables:us-east-1:123:bucket/my-tb"
        )

    def test_mediaconnect(self):
        from ministack.core.iam_actions import extract_resource_arn

        assert "mediaconnect" in extract_resource_arn(
            "mediaconnect", "GET", "/v1/flows/flow-123", {}, b"", {}, "us-east-1", "123"
        )
    ...
    def test_ec2_instance(self):
        ...
        assert (
            extract_resource_arn("ec2", "POST", "/", {}, b"", {"InstanceId.1": ["i-abc123"]}, "us-east-1", "123")
    ...
    def test_ec2_vpc(self):
        ...
        assert (
            extract_resource_arn("ec2", "POST", "/", {}, b"", {"VpcId": ["vpc-123"]}, "us-east-1", "123")
    ...
    def test_ec2_security_group(self):
        ...
        assert (
            extract_resource_arn("ec2", "POST", "/", {}, b"", {"GroupId": ["sg-123"]}, "us-east-1", "123")
            == "arn:aws:ec2:us-east-1:123:security-group/sg-123"
        )
    ...
    def test_ec2_no_resource(self):
        ...
        assert (
            extract_resource_arn("ec2", "POST", "/", {}, b"", {"Action": ["DescribeInstances"]}, "us-east-1", "123")
            == "*"
        )
    ...
    def test_iot_thing(self):
        ...
        assert (
            extract_resource_arn("iot", "GET", "/things/my-thing", {}, b"", {}, "us-east-1", "123")
    ...
    def test_iot_policy(self):
        ...
        assert (
            extract_resource_arn("iot", "GET", "/policies/my-policy", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:iot:us-east-1:123:policy/my-policy"
        )
    ...
    def test_iot_rule(self):
        ...
        assert (
            extract_resource_arn("iot", "GET", "/rules/my-rule", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:iot:us-east-1:123:rule/my-rule"
        )
    ...
    def test_apigateway_v2(self):
        ...
        assert (
            extract_resource_arn("apigateway", "GET", "/v2/apis/abc123", {}, b"", {}, "us-east-1", "123")
    ...
    def test_apigateway_v1(self):
        ...
        assert (
            extract_resource_arn("apigateway", "GET", "/restapis/xyz789", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:apigateway:us-east-1::/restapis/xyz789"
        )
    ...
    def test_bedrock_model(self):
        ...
        assert (
            extract_resource_arn(
                "bedrock-runtime", "POST", "/model/anthropic.claude-v2/invoke", {}, b"", {}, "us-east-1", "123"
            )
    ...
    def test_bedrock_agent(self):
        ...
        assert (
            extract_resource_arn("bedrock-agent", "GET", "/agents/AGENT123", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:bedrock:us-east-1:123:agent/AGENT123"
        )
    ...
    def test_bedrock_knowledge_base(self):
        ...
        assert (
            extract_resource_arn("bedrock-agent", "GET", "/knowledgebases/KB123", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:bedrock:us-east-1:123:knowledge-base/KB123"
        )
    ...
    def test_appsync_api(self):
        ...
        assert (
            extract_resource_arn("appsync", "GET", "/v1/apis/abc123", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:appsync:us-east-1:123:apis/abc123"
        )
    ...
    def test_appsync_datasource(self):
        ...
        assert (
            extract_resource_arn("appsync", "GET", "/v1/apis/abc/datasources/myds", {}, b"", {}, "us-east-1", "123")
            == "arn:aws:appsync:us-east-1:123:apis/abc/datasources/myds"
        )
    ...
    def test_resource_wildcard_in_arn(self):
        ...
        stmts = parse_policy_document(
            {
                "Statement": [
                    {"Effect": "Allow", "Action": "dynamodb:*", "Resource": "arn:aws:dynamodb:*:*:table/users*"}
                ]
            }
        )
        ctx = _ctx(action="dynamodb:PutItem", resource="arn:aws:dynamodb:us-east-1:123:table/users")
        assert evaluate(ctx, [stmts]).decision == "Allow"
        ctx2 = _ctx(action="dynamodb:PutItem", resource="arn:aws:dynamodb:us-east-1:123:table/orders")
...
class TestActionExtraction:
    def test_query_protocol(self):
        ...
        assert (
            extract_iam_action("monitoring", "POST", "/", {}, b"", {"Action": ["PutMetricData"]})
    ...
    def test_target_protocol(self):
        ...
        assert (
            extract_iam_action("dynamodb", "POST", "/", {"x-amz-target": "DynamoDB_20120810.PutItem"}, b"", {})
            == "dynamodb:PutItem"
        )
        assert (
            extract_iam_action("kms", "POST", "/", {"x-amz-target": "TrentService.Encrypt"}, b"", {}) == "kms:Encrypt"
        )
    ...
    def test_lambda_rest(self):
        ...
        assert (
            extract_iam_action("lambda", "POST", "/2015-03-31/functions/f/invocations", {}, b"", {})
...
class TestS3AdditionalChecks:
    ...
    _DELETE_BODY = (
        f'<Delete xmlns="{_NS}"><Object><Key>a.txt</Key></Object>'
    ...
    def test_copy_source_may_omit_the_leading_slash_and_name_a_version(self):
        checks = self._checks(
            "PUT",
            "/dst/key",
            {"x-amz-copy-source": "src/dir/a.txt?versionId=v3"},
            query={"uploadId": "u", "partNumber": "1"},
        )  # UploadPartCopy
    ...
    def test_copy_needs_read_on_the_source(self):
        stmts = parse_policy_document(
            {"Statement": [{"Effect": "Allow", "Action": "s3:PutObject", "Resource": "arn:aws:s3:::dst/*"}]}
        )
    ...
    def test_bypass_header_adds_the_bypass_action_on_the_object(self):
        assert self._checks("DELETE", "/b/k", self._BYPASS) == [("s3:BypassGovernanceRetention", "arn:aws:s3:::b/k")]
    ...
    def test_batch_delete_body_without_a_namespace_parses_too(self):
        ...
        assert self._checks("POST", "/b", body=body, query={"delete": ""}) == [("s3:DeleteObject", "arn:aws:s3:::b/y")]

    def test_batch_delete_skips_an_object_without_a_key(self):
        body = (
            b"<Delete><Object><VersionId>v1</VersionId></Object>"
...
class TestS3EnforcementSites:
    ...
    _BATCH = (
        f'<Delete xmlns="{_NS}"><Object><Key>a.txt</Key></Object><Object><Key>b.txt</Key></Object></Delete>'
    ).encode()
    ...
    def _vhost(bucket, path, method, headers, body, query):
        ...
        return asyncio.run(
            app_mod._handle_s3_vhost_request(f"{bucket}.localhost:4566", path, method, headers, body, query)
        )
    ...
    _ONLY_FIRST_KEY = {
        "Statement": [{"Effect": "Allow", "Action": "s3:DeleteObject", "Resource": "arn:aws:s3:::iam-sites-b/a.txt"}]
    }
    _EVERY_KEY = {
        "Statement": [{"Effect": "Allow", "Action": "s3:DeleteObject", "Resource": "arn:aws:s3:::iam-sites-b/*"}]
    }
    _WRITE_ONLY = {
        "Statement": [{"Effect": "Allow", "Action": "s3:PutObject", "Resource": "arn:aws:s3:::iam-sites-dst/*"}]
    }
    ...
    def test_vhost_batch_delete_is_denied_on_the_second_key(self, monkeypatch):
        ...
        assert seen == [
            ("s3:DeleteObject", "arn:aws:s3:::iam-sites-b/a.txt"),
    ...
    def test_vhost_copy_without_read_on_the_source_is_denied(self, monkeypatch):
        ...
        assert b"AccessDenied" in body
        assert seen == [
            ("s3:PutObject", "arn:aws:s3:::iam-sites-dst/k"),
    ...
    def test_a_primary_denial_stops_before_the_extra_checks(self, monkeypatch):
        seen = self._stub_evaluator(
            monkeypatch, {"Statement": [{"Effect": "Allow", "Action": "s3:GetObject", "Resource": "*"}]}
        )
...
class TestAccessDeniedResponse:
    ...
    def test_custom_error_code_preserved(self):
        ...
        s, h, b = access_denied_response(
            "s3", "s3:PutObject", "", "r1", error_code="ExpiredTokenException", message="Token expired"
        )
...
def test_simulate_custom_policy_allow(iam):
    ...
    resp = iam.simulate_custom_policy(PolicyInputList=[policy], ActionNames=["s3:GetObject", "s3:PutObject"])
...
def test_simulate_custom_policy_implicit_deny(iam):
    ...
    resp = iam.simulate_custom_policy(PolicyInputList=[policy], ActionNames=["ec2:RunInstances"])
...
def test_simulate_custom_policy_explicit_deny(iam):
    ...
    resp = iam.simulate_custom_policy(PolicyInputList=[policy], ActionNames=["s3:PutObject", "s3:DeleteBucket"])
...
def test_create_policy_rejects_missing_action(iam):
    with pytest.raises(ClientError) as exc:
        iam.create_policy(
            PolicyName="bad-policy-no-action",
...
def test_put_role_policy_rejects_malformed_document(iam):
    ...
    try:
        with pytest.raises(ClientError) as exc:
            iam.put_role_policy(RoleName="validation-test-role-2", PolicyName="bad", PolicyDocument="not json")
        assert exc.value.response["Error"]["Code"] == "MalformedPolicyDocument"
    finally:
...
class TestExtractResourceArn:
    ...
    def test_sqs_queue_url_from_json_body(self):
        ...
        assert self._arn("sqs", body=body) == ("arn:aws:sqs:us-east-1:000000000000:my-queue")
    ...
    def test_sqs_queue_url_from_query_form_still_works(self):
        ...
        assert self._arn("sqs", query=query) == ("arn:aws:sqs:us-east-1:000000000000:my-queue")

    def test_sqs_queue_from_request_path(self):
        # Query-protocol callers address the queue by path.
        assert self._arn("sqs", path="/000000000000/my-queue") == ("arn:aws:sqs:us-east-1:000000000000:my-queue")
    ...
    def test_kms_decrypt_resolves_the_key_from_the_ciphertext(self):
        ...
        assert self._arn("kms", body=body) == (f"arn:aws:kms:us-east-1:000000000000:key/{key_id}")
    ...
    def test_kms_explicit_key_id_wins_over_the_ciphertext(self):
        ...
        blob = "1234abcd-12ab-34cd-56ef-1234567890ab".encode() + b"\x00" * 48 + b"payload"
        body = json.dumps(
            {
                "KeyId": "alias/my-key",
                "CiphertextBlob": base64.b64encode(blob).decode(),
        ...
        assert self._arn("kms", body=body) == ("arn:aws:kms:us-east-1:000000000000:alias/my-key")
    ...
    def test_ssm_parameter_name_from_json_body(self):
        ...
        assert self._arn("ssm", body=body) == ("arn:aws:ssm:us-east-1:000000000000:parameter/app/db")

    def test_cloudwatch_alarm_name_from_json_body(self):
        body = json.dumps({"AlarmName": "cpu-high"}).encode()
        assert self._arn("monitoring", body=body) == ("arn:aws:cloudwatch:us-east-1:000000000000:alarm:cpu-high")
...
def test_lambda_build_config_returns_a_config_not_an_error(monkeypatch):
    ...
    monkeypatch.setattr(app_mod, "AUTH", True, raising=False)
    config = lambda_svc._build_config(
        "f",
        {
            "Runtime": "nodejs22.x",
            "Handler": "index.handler",
...
def test_lambda_execution_credentials_resolve_to_configured_role(monkeypatch):
    ...
    try:
        credentials = lambda_svc.execution_credentials(
            {
                ...
                "FunctionArn": ("arn:aws:lambda:us-east-1:000000000000:function:appointment-mark-canceled"),
        ...
        assert sts_svc._sessions[access_key]["Arn"].startswith(f"arn:aws:sts::000000000000:assumed-role/{role_name}/")
        assert (
            enforce(
                access_key,
...
def test_role_session_uses_account_from_session_arn(monkeypatch):
    ...
    sts_svc._sessions["ASIASESSIONACCOUNT"] = {"Arn": f"arn:aws:sts::{account_id}:assumed-role/{role_name}/lambda"}
    try:
