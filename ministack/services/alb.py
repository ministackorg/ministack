...
async def _authenticate_oidc(action, method, path, headers, body, query_params, listener_port):
    ...
    if path == OIDC_CALLBACK_PATH:
        ...
        if not code or pending is None:
            # A callback with no matching request is a replayed or forged one.
        ...
        # Only what the X-Amzn-Oidc-* headers are built from. The claims are
        # already inside the ID token; storing them again would double the
        # session for nothing and push it over another shard boundary.
        session = {
            "sub": claims.get("sub", ""),
    ...
    if session and int(session.get("exp", 0)) > time.time():
        # AWS strips any client-supplied copies of these before setting its own,
        # so a caller cannot forge an identity by sending the header directly.
        headers["x-amzn-oidc-identity"] = session.get("sub", "")
        headers["x-amzn-oidc-accesstoken"] = session.get("access_token", "")
        headers["x-amzn-oidc-data"] = session.get("id_token", "")
        return None
