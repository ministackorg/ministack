...
def _test_state(data):
    ...
    try:
        if state_type == "Pass":
            ...
            fail_error, fail_cause = _resolve_fail_error_cause(state_def, input_data, ctx)
            result = {
                "status": "FAILED",
                "error": fail_error,
                "cause": fail_cause,
...
def _run_execution(exec_arn):
    ...
    ctx = {
        ...
        # the whole execution, except a Map iteration or a Parallel branch:
        # each runs against its own deep copy of this dict, so an `Assign`
        # made inside one goes out of scope once the Map/Parallel state
        # completes (per AWS). A state-level `Assign` on the Map/Parallel
        # itself still writes to this parent dict, applied after every
        # iteration/branch has finished.
        "variables": {},
    }
    ...
    try:
        while current_name and execution["status"] == "RUNNING":
            ...
            if state_type == "Fail":
                fail_error, fail_cause = _resolve_fail_error_cause(state_def, current_input, ctx)
                raise _ExecutionError(fail_error, fail_cause)
...
def _execute_wait(state_def, raw_input, ctx=None):
    if _state_query_language(state_def, ctx) == "JSONata":
        # `Seconds`/`Timestamp` carry the `{% %}` expression directly in JSONata
        # mode — there is no `SecondsPath`/`TimestampPath` (AWS drops every
        # `*Path` field for the base field it names once QueryLanguage is JSONata).
        _apply_jsonata_wait(state_def, raw_input, ctx)
        output = _apply_jsonata_output(state_def, raw_input, ctx, default=raw_input)
        _apply_state_assign(state_def, raw_input, ctx, result=output)
        return output, _next_or_end(state_def)
...
def _apply_jsonata_wait(state_def, raw_input, ctx):
    """Evaluate a JSONata Wait state's `Seconds`/`Timestamp`. A resolved value of
    the wrong type is a JSONata type incompatibility — AWS's States.QueryEvaluationError,
    catchable and retryable — not an uncaught crash in the sleep/parse math below."""
    if "Seconds" in state_def:
        seconds = _apply_jsonata_template(state_def["Seconds"], raw_input, ctx)
        if isinstance(seconds, bool) or not isinstance(seconds, (int, float)):
            raise _ExecutionError(
                "States.QueryEvaluationError",
                f"The Wait state's Seconds field must evaluate to a number, not {seconds!r}",
            )
        _scaled_sleep(seconds)
    elif "Timestamp" in state_def:
        timestamp = _apply_jsonata_template(state_def["Timestamp"], raw_input, ctx)
        if not isinstance(timestamp, str):
            raise _ExecutionError(
                "States.QueryEvaluationError",
                f"The Wait state's Timestamp field must evaluate to a string, not {timestamp!r}",
            )
        _sleep_until(timestamp)
...
def _execute_parallel(state_def, raw_input, execution, ctx):
    ...
    def run_branch(idx, branch):
        try:
            # A branch's own deep copy: an Assign made inside it must go out of
            # scope when the Parallel state completes (per AWS), not leak into
            # the parent execution's variables.
            branch_ctx = copy.deepcopy(ctx)
            results[idx] = _run_sub_machine(
                branch.get("States", {}),
                branch.get("StartAt"),
                effective, execution, branch_ctx)
        except Exception as exc:
            errors[idx] = exc
    ...
    output = _apply_map_or_parallel_output(state_def, raw_input, ctx, query_language, results)
    return output, _next_or_end(state_def)
...
def _execute_map(state_def, raw_input, execution, ctx):
    ...
    if query_language == "JSONata":
        effective = _apply_jsonata_arguments(state_def, raw_input, ctx)
        # `Items` replaces `ItemsPath` in JSONata mode; missing it means the
        # Arguments-computed input is itself the array (mirrors ItemsPath's "$" default).
        items = (_apply_jsonata_template(state_def["Items"], raw_input, ctx)
                if "Items" in state_def else effective)
    else:
        ...
        effective = _apply_input_path(state_def, raw_input)
        items = _resolve_path(state_def.get("ItemsPath", "$"), effective)
...
def _apply_map_or_parallel_output(state_def, raw_input, ctx, query_language, results):
    """Turn a Map's per-item results or a Parallel's per-branch results into the
    state's output, the way `_execute_task` turns a Task's result into its output."""
    if query_language == "JSONata":
        output = _apply_jsonata_output(state_def, raw_input, ctx, result=results, default=results)
        _apply_state_assign(state_def, raw_input, ctx, result=results)
        return output
    result = _apply_result_selector(state_def, results)
    output = _apply_result_path(state_def, raw_input, result)
    return _apply_output_path(state_def, output)
...
def _apply_jsonata_output(state_def, raw_input, ctx=None, result=jsonata.UNDEFINED,
                          error_output=jsonata.UNDEFINED, default=None):
...
def _apply_state_assign(state_def, raw_input, ctx, result=jsonata.UNDEFINED,
                        error_output=jsonata.UNDEFINED):
...
def _apply_jsonata_template(value, raw_input, ctx=None, result=jsonata.UNDEFINED,
                            error_output=jsonata.UNDEFINED):
...
def _resolve_fail_error_cause(state_def, raw_input, ctx=None):
    """A Fail state's `Error`/`Cause`, evaluating `{% %}` JSONata templates when the
    state's query language is JSONata — the same "no more `*Path` fields" rule that
    turns `TimeoutSecondsPath` into a `{% %}` expression inside `TimeoutSeconds`
    applies to `ErrorPath`/`CausePath` and the plain `Error`/`Cause` fields here."""
    error = state_def.get("Error", "States.Fail")
    cause = state_def.get("Cause", "")
    if _state_query_language(state_def, ctx) == "JSONata":
        error = _apply_jsonata_template(error, raw_input, ctx)
        cause = _apply_jsonata_template(cause, raw_input, ctx)
    return error, cause


# The AWS Context Object has exactly these top-level fields. `variables` and
# `QueryLanguage` are MiniStack's own control state — this run's Assign store
# and query-language resolution — not something AWS exposes to `$states.context`.
_AWS_CONTEXT_FIELDS = ("Execution", "State", "StateMachine", "Task", "Map")

# How deep an expression may nest before the evaluator gives up. The interpreter itself has no
# ceiling, so this is where MiniStack picks one: far enough above what a state machine writes to
# leave a legitimate expression alone, and far enough below Python's own stack that going past it
# fails the execution instead of crashing the worker.
_MAX_JSONATA_DEPTH = 200


def _evaluate_jsonata(expression, raw_input, ctx=None, result=jsonata.UNDEFINED,
                      error_output=jsonata.UNDEFINED):
    """Evaluate one `{% … %}` body against the state input, with the reserved `$states` bound.

    Workflow variables are bound as bare `$name` so a `Condition` or an `Output` reads whatever
    an earlier `Assign` wrote. `$states` is bound last because AWS reserves the name, so a
    variable called `states` cannot shadow it.
    """
    bindings = dict((ctx or {}).get("variables", {}))
    bindings["states"] = {
        "input": raw_input,
        "result": result,
        "errorOutput": error_output,
        "context": {k: v for k, v in (ctx or {}).items() if k in _AWS_CONTEXT_FIELDS},
    }
    try:
        value = jsonata.evaluate(expression.strip(), raw_input, bindings,
                                 max_depth=_MAX_JSONATA_DEPTH)
    except jsonata.JsonataError as exc:
        raise _ExecutionError(
            "States.QueryEvaluationError",
            f"{exc} while evaluating '{expression.strip()}'",
        )
    if value is jsonata.UNDEFINED:
        # JSON cannot represent an undefined value, so AWS fails the execution rather than
        # writing a null the author never asked for.
        raise _ExecutionError(
            "States.QueryEvaluationError",
            f"The JSONata expression '{expression.strip()}' returned an undefined value",
        )
    try:
        json.dumps(value, allow_nan=False, default=_undefined_reads_as_null)
    except (TypeError, ValueError):
        # A function value or a non-finite number (Infinity/NaN) is a value JSON cannot
        # represent. AWS documents this as States.QueryEvaluationError — catchable and
        # retryable — rather than letting it surface as an uncaught json.dumps failure
        # once the caller serializes the execution output.
        raise _ExecutionError(
            "States.QueryEvaluationError",
            f"The JSONata expression '{expression.strip()}' returned a value JSON cannot represent",
        )
    # Round-tripped through the interpreter's own renderer so the value handed on is JSON's:
    # a large integer keeps its digits instead of turning into `1e+17`, and an `UNDEFINED` a
    # builtin such as `$match` left inside the result reads as JSON.stringify reads it.
    return json.loads(jsonata.stringify(value))


def _undefined_reads_as_null(value):
    """What `json.dumps` writes for a value it cannot serialize on its own.

    A JSONata `undefined` nested in a document has a JSON spelling and `$match` produces one for
    an optional group that did not participate; a function value has no spelling at all.
    """
    if value is jsonata.UNDEFINED:
        return None
    raise TypeError(f"{value!r} is not JSON")
