...
### Added
- **IoT Core — provisioning templates, and `AWS::IoT::ProvisioningTemplate` in CloudFormation** — the five template operations all answered `InvalidRequestException: Unsupported IoT path`, and a stack carrying the resource type rolled back with `Unsupported resource type`, so a just-in-time-provisioning stack could not deploy at all — its Lambda came up with no template referencing it. `CreateProvisioningTemplate`, `DescribeProvisioningTemplate`, `ListProvisioningTemplates` (unpaginated), `UpdateProvisioningTemplate` and `DeleteProvisioningTemplate` now serve the wire shapes a live probe of the real service measured — duplicate creates answer `ResourceAlreadyExistsException`, unknown names `ResourceNotFoundException` (`Unable to find template named ...`), update answers 200 with an empty body, describe omits an unset `description`, template names are validated (1-36 chars of `[0-9A-Za-z_-]`), and a `templateBody` without an `AWS::IoT::Certificate` resource is refused with the real service's message — and persist with the IoT store when `PERSIST_STATE=1`. Storage + CRUD only, deliberately: the fleet-provisioning MQTT workflow (`$aws/provisioning-templates/...`) is not implemented, `provisioningRoleArn` is presence-checked but not resolved (MiniStack does not validate IAM role ARNs elsewhere either), and versions are not modeled (`defaultVersionId` is always 1). The CloudFormation provisioner drives the real API, applies `Description` / `Enabled` / `ProvisioningRoleArn` / `PreProvisioningHook` / `TemplateBody` changes in place under the same physical id, generates a physical name when `TemplateName` is omitted, and replaces the template on a `TemplateName` change the way `AWS::IoT::Policy` does. Contributed by @iot-rocket.
