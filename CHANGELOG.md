...
### Added
- **CloudFormation — in-place update handlers for the resource types templates actually update** — 106 of the 132 registered types had no update handler, so a stack update fell through to re-running create, which destroyed everything CloudFormation doesn't own on AWS: a Lambda function lost its published versions, aliases, resource policy and tags; a DynamoDB table its items; an SQS queue its messages; a log group its streams; an IAM role every policy attached from outside the template. `AWS::Lambda::Function`, `AWS::DynamoDB::Table`, `AWS::ApiGateway::{RestApi,Resource,Method,Stage}`, `AWS::IoT::TopicRule`, `AWS::SNS::Topic`, `AWS::SQS::Queue`, `AWS::Logs::LogGroup`, `AWS::S3::BucketPolicy`, `AWS::IAM::Role` and `AWS::IAM::ManagedPolicy` now update in place through their service's own mutation APIs, keeping the physical id. Genuinely create-only identity properties (names, an API Gateway Resource's path, a REST API's OpenAPI `Body`) replace via create-new-then-delete-old; a `KeySchema` or LSI change on an explicitly named table refuses the update the way AWS refuses replacing a custom-named resource; a changed `AWS::IAM::ManagedPolicy` document becomes a new default policy version on the same ARN, pruning at the five-version cap like the CloudFormation handler. `AWS::ApiGateway::Deployment` stays create-only, matching AWS's immutable-snapshot semantics.
...
### Fixed
- **CloudFormation — a resource that cannot be deleted now fails the operation instead of leaking silently** — `_delete_resource` answered a missing delete handler with a log warning and returned as if the delete had happened, so `DeleteStack` landed `DELETE_COMPLETE` while the resources lived on, and a rollback reported `(UPDATE_)ROLLBACK_COMPLETE` having undone nothing. Five of the 132 registered types had a create handler but no delete. A missing handler now raises: `AWS::Lambda::Version` gained a real delete handler (and its `Ref` now returns the qualified version ARN, as on AWS — which is also what lets the delete find its version), `AWS::AppSync::GraphQLSchema` gained one that removes the stored schema, and the intentionally stateless `AWS::CloudFormation::WaitCondition`/`WaitConditionHandle` carry an explicit no-op so the raise stays reserved for types that really leak. `DeleteStack` keeps deleting the remaining resources after a failure and then lands `DELETE_FAILED` with a stack-level event — the failed resources stay on the stack (marked `DELETE_FAILED`, so a retried `DeleteStack` picks them up again), everything that did delete is gone, and the exports stay with the still-existing stack. A rollback whose delete fails lands `ROLLBACK_FAILED` / `UPDATE_ROLLBACK_FAILED` instead of pretending the rollback completed. The update path that deletes resources removed from the template still lands `UPDATE_COMPLETE` — the behaviour we have observed on AWS when the cleanup phase cannot delete — but now emits the resource-level `DELETE_FAILED` event it lacked.
