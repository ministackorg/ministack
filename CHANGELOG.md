...
- **API Gateway (REST) — a non-proxy `AWS` Lambda integration returns the raw handler output** — it shared the AWS_PROXY response path, so a handler's plain dict was read as a `{statusCode, headers, body}` envelope: its `statusCode` became the HTTP status and the rest was dropped. The output is now the JSON body with the integration response's status (200 by default); a function error is `502 {"message": "Internal server error"}`. Mapping templates are not modeled. Contributed by @iot-rocket.
