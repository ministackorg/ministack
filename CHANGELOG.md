...
- **Cognito — `USER_SRP_AUTH` checks the password** — `PASSWORD_VERIFIER` answered random `SALT` and `SRP_B` values and accepted any response, so a wrong password, a missing `TIMESTAMP` or a disabled or unconfirmed user all got tokens. The challenge now carries real SRP parameters with a stable salt per user, and the response is refused with `NotAuthorizedException` unless its signature proves the stored password. `TIMESTAMP` and `PASSWORD_CLAIM_SIGNATURE` are validated as AWS validates them, a temporary password leads to `NEW_PASSWORD_REQUIRED`, and `AdminInitiateAuth` accepts the flow. A client without `ALLOW_USER_SRP_AUTH`, an unknown user under `LEGACY`, a disabled and an unconfirmed user are refused at `InitiateAuth`. The built-in `PASSWORD_VERIFIER` step of `CUSTOM_WITH_SRP` gets the same check. Contributed by @iot-rocket.
- **Cognito — an unconfirmed user cannot sign in with a password** — `USER_PASSWORD_AUTH` and `ADMIN_USER_PASSWORD_AUTH` issued tokens to a user who never confirmed the sign-up. Both now answer `UserNotConfirmedException` before the password is checked, as AWS does for a right and a wrong password alike. Contributed by @iot-rocket.
### Added
