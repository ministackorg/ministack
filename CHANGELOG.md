...
- **SNS — subscription filter policies using `{"exists": false}` never matched** — a subscription whose `FilterPolicy` asked for an *absent* message attribute (for example `{"store": [{"exists": false}]}`) received nothing at all: a message attribute missing from `Publish` short-circuited the policy evaluation to "no match" before the `exists` rule was ever considered, so every notification was silently dropped. The `exists` operator is now evaluated against attribute presence rather than value, matching AWS (and MiniStack's own EventBridge pattern matcher): a missing attribute satisfies `{"exists": false}` and a present one still does not.
