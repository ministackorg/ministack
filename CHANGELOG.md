...
### Added
- **ECS — an `awsvpc` task's network interface names its subnet** — the `ElasticNetworkInterface` attachment carried only `privateIPv4Address`. It now also carries `subnetId`, read from the `awsvpcConfiguration` the request placed the task in, in either casing so a service defined through CloudFormation is covered too. The `Attachment` reference names four details for an elastic network interface; the network interface id and the MAC address are still not reported. Contributed by @iot-rocket.
...
- **Lambda — a FIFO event source keeps one batch in flight** — dispatching SQS batches off the poll thread meant several batches of one FIFO queue could run at once and reorder a message group, since a batch is built across groups and Amazon SQS "ensures that messages in the same group are delivered to Lambda in order". A FIFO event source is now held to one batch at a time whatever `ScalingConfig.MaximumConcurrency` says. Standard queues are unaffected.
...
- **ECS — a starting `awsvpc` task reports `PROVISIONING`** — every Docker-backed task was `PENDING` from `RunTask`, so a consumer waiting for the state a task sits in while its network interface is set up waited forever. An `awsvpc` task now starts `PROVISIONING`, "for tasks that use the `awsvpc` network mode, the elastic network interface needs to be provisioned", and every other network mode starts `PENDING`. The new state counts against a service's desired capacity like the others, so reconciliation does not launch a duplicate; `pendingCount` and `pendingTasksCount` still count `PENDING` alone, which is what the API model says they are. Reported by @iot-rocket.
- **ECS — only an `awsvpc` task reports an elastic network interface** — every task got an `ElasticNetworkInterface` attachment once its container was up, whatever its network mode, while the Task reference describes `attachments` as the adapter a task has "if the task uses the `awsvpc` network mode": a `bridge` or `host` task on AWS reports none and is reached through its container instance. A task in those modes now reports an empty list, and the address stays available to the target-group sync in every mode. Contributed by @iot-rocket.
