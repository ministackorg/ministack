...
- **Batch `SubmitJob` succeeds for jobdefs the host can't run** — a jobdef requesting `VCPU=4, MEMORY=31000` on a 2 vCPU / 7 GiB host would return `SUCCEEDED` immediately (MiniStack's Batch is metadata-only) even though the same workload could never start on real Docker. MiniStack now compares the jobdef's `resourceRequirements` against the host's installed VCPU and memory (via `os.cpu_count()` and `os.sysconf`) and refuses with a 400 `ClientException` carrying the host's actual limits. Deliberate divergence from real AWS Batch — AWS doesn't run out of host resources, the local emulator does. Dimensions that can't be measured (Windows, missing `sysconf`) are skipped so the gate never false-positives. Reported by @michael-denyer.
