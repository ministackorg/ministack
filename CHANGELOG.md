...
### Added
- **Step Functions alias API** — `CreateStateMachineAlias`, `UpdateStateMachineAlias`, `DeleteStateMachineAlias`, `DescribeStateMachineAlias`, `ListStateMachineAliases`, plus `DescribeStateMachine` support for alias ARNs (`arn:…:stateMachine:<name>:<aliasName>`). Aliases let callers address a state machine by a stable pointer whose `routingConfiguration` distributes invocations across one or two versions by weight. Validation matches AWS: alias name regex `^[0-9A-Za-z_-]+$` length 1-80, routing 1-2 entries with int `weight` in `[0, 100]` summing to 100, no duplicate version ARNs, every referenced version must exist on the same state machine. `DeleteStateMachineVersion` now scans for alias references and raises `ConflictException` when an alias still routes to the version — the alias-awareness cross-check deferred from the versioning PR. Same terraform-provider-aws v6 motivation — the provider unconditionally calls `ListStateMachineAliases` on every `aws_sfn_state_machine` refresh, so without this any `terraform plan` returned `InvalidAction`.
- **Step Functions versioning API** — `PublishStateMachineVersion`, `DeleteStateMachineVersion`, `ListStateMachineVersions`, plus `DescribeStateMachine` support for qualified version ARNs (`arn:…:stateMachine:<name>:<N>`) returning the snapshot captured at publish time. `CreateStateMachine` now accepts the `publish=True` parameter, returning `stateMachineVersionArn` for the auto-published v1 in the same call. AWS-style optimistic concurrency via `revisionId`: every Create/Update rotates the state machine's `revisionId`, and `PublishStateMachineVersion`'s optional `revisionId` is treated as a precondition — supplying a stale value raises `ConflictException`. Version numbers are monotonic per state machine and never reused after delete (a `lastVersionNumber` high-water mark on the base state machine enforces this, matching AWS's documented behaviour). Without these actions, terraform-provider-aws v6 fails on every `aws_sfn_state_machine` refresh because it unconditionally calls `ListStateMachineVersions` regardless of whether versioning was opted into.
