...
### Changed
- **Persistence — every saved snapshot is restored through the registry** — a service restored its own state as a side effect of being imported, so a module the router never reached kept its state on disk, and the boot path carried a hand-maintained list of eager imports to paper over it. The registry now drives one restore loop, and the runtime work a restore implies — the RDS container respawn, the ECS service reconciler, the Pipes and Lambda event-source pollers, the OpenSearch data planes — runs from each module's `load_persisted_state` instead of its import. Contributed by @jgrumboe.
...
- **DynamoDB — `ImportTable` reads the S3 source** — the import created the destination table, reported `COMPLETED` after a moment and imported nothing, whatever the format: the source objects were never opened. CSV and `DYNAMODB_JSON` sources, compressed with GZIP or not, are now read from the prefix and written into the table, with `ProcessedItemCount`, `ImportedItemCount`, `ProcessedSizeBytes` and `ErrorCount` reported as the import runs. A malformed object is one logged error and the rest of the source continues; an item that fails validation ends the import `FAILED` with `ItemValidationError`, and per-item errors are written to the `/aws-dynamodb/imports` log group as AWS documents them. A missing bucket or prefix fails the import and leaves no table behind. `ION` sources and `ZSTD` compression are refused with a stated reason rather than reported as an empty success. Reported by @gakuto-cw21.
- **Persistence — a restored snapshot keeps every account's state** — ElastiCache lost its default parameter groups on any boot without a state file, and Organizations, OpenSearch, CUR and the CloudFront KeyValueStore dropped every account but the default one, because a scoped store's truthiness and iteration answer for the calling account and the loader runs with no request scope.
