...
### Fixed
- **Glue — a Spark job's loopback endpoints reach MiniStack from inside the job container** — a user `--conf` value pointing at `localhost`, `127.0.0.1` or `host.docker.internal` on the gateway port (an Iceberg REST catalog URI, an S3A endpoint) was handed to the container verbatim, where those names do not resolve to MiniStack, so the Iceberg client fell back to a dead endpoint and the job failed on name resolution. The host part of such values is now rewritten to the resolved gateway address — any other host or port passes through untouched, so an external REST catalog keeps working — the container gets `host.docker.internal` mapped via `host-gateway` for the Linux engine, and the endpoint port honours `GATEWAY_PORT` (only `EDGE_PORT` was read).
