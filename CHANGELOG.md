...
### Added
- **Lambda — SnapStart** — the `SnapStart` parameter on `CreateFunction` and `UpdateFunctionConfiguration` was silently swallowed: every read answered `{ApplyOn: None, OptimizationStatus: Off}` regardless of what was set, so a Terraform/CDK deployment that enables SnapStart saw permanent drift and re-applied on every plan. The setting now round-trips: `$LATEST` echoes the configured `ApplyOn` with `OptimizationStatus: Off` (a snapshot only ever exists per published version, as on AWS), and publishing a version with `ApplyOn=PublishedVersions` stamps the version `OptimizationStatus: On`, walking it `Pending` → `Active` the way AWS does while the snapshot is created — on `PublishVersion`, `Publish=true` at create, and `Publish=true` on `UpdateFunctionCode` alike. The documented constraints are enforced before anything mutates: only Java 11+, Python 3.12+ and .NET 8+ zip runtimes (container images are accepted; AWS validates those at snapshot time), refused with `InvalidParameterValueException` otherwise — including moving an enabled function to an unsupported runtime — and ephemeral storage above 512 MB is refused in both directions. No Firecracker snapshot exists locally, but the *mechanism* is emulated, not just the shape: publishing a SnapStart version initializes its execution environment right then — a warm worker for Python, a pooled RIE container for Java/.NET/Image — so the version walks `Pending` → `Active` while init runs, invoking it while `Pending` answers `ResourceConflictException` as documented, the first invoke lands on the pre-warmed environment, and an init that raises fails the *publish* (`State: Failed`, `StateReasonCode: FunctionError`) instead of surfacing at first invoke. Python runtime hooks run too: `snapshot-restore-py` registrations execute during a published version's init — before-snapshot hooks in reverse registration order, then after-restore hooks in order, never for `$LATEST` — so hook code is exercised locally; a hook that raises fails the publish. Java CRaC and .NET hooks live inside the RIE's bootstrap, which MiniStack does not own, and do not fire.
