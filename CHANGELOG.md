...
### Added
- **SNS — `GetPlatformApplicationAttributes` / `SetPlatformApplicationAttributes`** — a platform application could be created and deleted but never read back or updated: both actions answered `400 InvalidAction`, so `terraform plan` on an `aws_sns_platform_application` failed at refresh (`reading SNS Platform Application (...): Unknown action: GetPlatformApplicationAttributes`) and the resource could never reach a clean state. Both now answer with the documented shapes — `Get` returns the attribute map as `entry` key/value pairs, `Set` merges the supplied entries (`PlatformCredential`, `PlatformPrincipal`, the `Event*` topic ARNs, `*FeedbackRoleArn`, `Enabled`, ...) and returns only `ResponseMetadata` — with `InvalidParameter` (400) for a missing `PlatformApplicationArn` or `Attributes` and `NotFound` (404) for an unknown application. `CreatePlatformApplication` also seeds `Enabled=true`, as AWS reports for a newly created application.

### Fixed
- **IAM — a resource-scoped policy works for JSON-protocol services (`AUTH=true`)** — `extract_resource_arn` read SQS, ACM, SSM and CloudWatch parameters from the query form only. SQS moved to the JSON protocol in 2023 and the others accept it, so for current SDKs nothing reached the query params, the resource fell back to `*`, and no queue-, certificate-, parameter- or alarm-scoped statement could match: every least-privilege policy for those services denied every call, while `Resource: "*"` allowed them. Those branches now read the request body and fall back to the query form, and SQS also resolves a queue addressed by request path. Reported by @rsimples.
- **IAM — `kms:Decrypt` resolves the key from the ciphertext (`AUTH=true`)** — `Decrypt` and `ReEncrypt` carry no `KeyId` for a symmetric key, since AWS recovers it from metadata in the ciphertext blob and evaluates IAM against that key's ARN. MiniStack fell back to `*`, so a policy naming the key allowed `Encrypt` and denied `Decrypt` for the same key. The key id is now recovered from the blob the same way the KMS handler recovers it. Reported by @rsimples.
- **Lambda — `CreateFunction` with an unresolvable role answers 400, not 500 (`AUTH=true`)** — the role check sat inside `_build_config`, which is declared to return a config dict, so its error tuple was assigned to `config` and the next attribute read raised `AttributeError: 'tuple' object has no attribute 'get'`; the client got `500 InternalError` with nothing about the role, and a half-created record was left behind for a background thread to trip over. The role is now validated before anything is built or stored, and the caller answers `400 InvalidParameterValueException`. Reported by @rsimples.
- **Cognito — group membership records the resolved Username** — `AdminAddUserToGroup` stored the caller-supplied name in the group's member list, but the list is read back as `_users` keys. In a pool with `UsernameAttributes = ["email"]` that name is an email alias, not the key, so `ListUsersInGroup` returned an empty list for a group that had just accepted the user, `AdminRemoveUserFromGroup` did not match the entry it had added, and `DeleteGroup` left the group on the user's own group list. Both operations now key on the resolved Username.
- **Cognito — `AdminDeleteUser` deletes the user an email alias resolves to** — in a pool with `UsernameAttributes = ["email"]`, `AdminCreateUser` stores the user under a generated UUID Username and keeps the email as an alias attribute, as real Cognito does. `AdminDeleteUser` resolved the alias correctly but then deleted by the caller-supplied name, which is not the key, so the call raised `KeyError` and answered `500 InternalError` while the user survived. A seed or test fixture that deletes and creates the same user therefore failed with `UsernameExistsException` on every run after the first. The delete now uses the resolved Username.
...
### Added
- **Cognito — link a federated identity to an existing user (`AdminLinkProviderForUser`)** — the call answered `InvalidAction`, so the standard account-linking `PreSignUp` trigger could not be exercised: a federated sign-in whose verified email already belonged to a native user always created a second `EXTERNAL_PROVIDER` profile with a new `sub`, and applications that resolve their records by `sub` saw one person as two accounts. `AdminLinkProviderForUser` and `AdminDisableProviderForUser` now link and unlink an external identity, the linked profile carries the `identities` attribute (`userId`, `providerName`, `providerType`, `issuer`, `primary`, `dateCreated`) and an `identities` ID-token claim, and a SAML or OIDC sign-in that matches a link signs in to the linked profile — keeping its `sub`, refreshing its mapped attributes and skipping the `PreSignUp` trigger — instead of minting a `{Provider}_id` profile. Linking an identity that already signed in is refused `AliasExistsException`, and the documented limit of five linked identities per profile is enforced. Reported by @rsimples.
- **RDS — RDS Proxy (control plane)** — `CreateDBProxy` and the twelve other proxy operations answered `InvalidAction`, so a Terraform, CloudFormation or Crossplane definition of an `aws_db_proxy` could not be applied at all. Proxies, proxy endpoints, the default target group and registered instance and cluster targets now create, describe, modify and delete with the documented shapes, defaults (`IdleClientTimeout` 1800, `MaxConnectionsPercent` 100 or 10 on SQL Server, `ConnectionBorrowTimeout` 120) and faults (`DBProxyAlreadyExistsFault`, `DBProxyNotFoundFault`, `DBProxyEndpointNotFoundFault`, `DBProxyTargetGroupNotFoundFault`, `DBProxyTargetAlreadyRegisteredFault`, `DBProxyQuotaExceededFault`, `DBProxyEndpointQuotaExceededFault`, `InvalidDBProxyEndpointStateFault`, `InvalidSubnet`). Creating a proxy also creates its default read/write endpoint and its `default` target group, as AWS does; the default endpoint refuses modification and deletion, `Marker` / `MaxRecords` paginate, and the documented constraints are enforced (`IdleClientTimeout` 1-28,800, `MaxConnectionsPercent` 1-100, `ConnectionBorrowTimeout` 0-300, `MaxIdleConnectionsPercent` between 0 and `MaxConnectionsPercent` and defaulting to half of it, `Auth` required unless `DefaultAuthScheme` is `IAM_AUTH`, one target per proxy, and a target whose engine the proxy's `EngineFamily` supports). This is metadata only: no connection pooling happens and nothing listens on the proxy endpoint hostname. Reported by @jmreicha.
