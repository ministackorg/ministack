...
- **CloudFormation — change sets report which properties replace** — for 21 resource types a change set now follows the type's schema, checked against real change sets: a create-only property is `Always` and makes the change `Replacement: True`, DynamoDB `KeySchema` and Lambda `DurableConfig` stay `Conditionally`, every other property is `Never`. An SQS queue or SNS topic rename and a Lambda layer version edit replace; an SSM value, a Lambda timeout or a Cognito user pool edit does not. Other types keep the `Conditionally` answer. Only the report changes, not how an update runs, and no `PolicyAction` is sent yet. Contributed by @iot-rocket.
