...
### Fixed
- **Four fields accepted on write but never reported on read** — each made the Terraform provider see a resource as unconfigured, so it planned the same change on every run and the change survived being applied. `CreateEventSourceMapping` never returned `EventSourceMappingArn`, which is the ARN the provider hands to `ListTags`, so no mapping ever reported a tag (completing #442). S3 Control’s `ListTagsForResource` wrapped tags in `<member>` where the model declares `locationName: "Tag"` — boto3 tolerates it, aws-sdk-go-v2 parses an empty list — so a bucket’s tags read back empty (completing #447). Kinesis stored the `KeyId` given to `StartStreamEncryption` and echoed it from no describe path. RDS never parsed, stored or serialised `ServerlessV2ScalingConfiguration` (on create *or* modify, so an existing cluster could not be repaired by an update either) and hardcoded `PerformanceInsightsRetentionPeriod` to 7 before dropping it from the response. Contributed by @jbschooley.
- **Cognito — a user pool keeps `UserAttributeUpdateSettings` and a disabled software-token MFA config** — the update settings were neither stored on create nor listed as updatable, and `GetUserPoolMfaConfig` dropped `SoftwareTokenMfaConfiguration` whenever it was falsy, which is exactly what a caller sends to turn software-token MFA off (`enabled = false` serialises with the false field omitted). Both read back absent, so Terraform proposed the same two blocks on every plan and applying them did not settle. A config that was explicitly set is now reported, with `Enabled` normalised to a boolean, while one that was never set stays absent. Contributed by @jbschooley.
- **Four fields accepted on write but never reported on read** — each made the Terraform provider see a resource as unconfigured, so it planned the same change on every run and the change survived being applied. `CreateEventSourceMapping` never returned `EventSourceMappingArn`, which is the ARN the provider hands to `ListTags`, so no mapping ever reported a tag (completing #442). S3 Control’s `ListTagsForResource` wrapped tags in `<member>` where the model declares `locationName: "Tag"` — boto3 tolerates it, aws-sdk-go-v2 parses an empty list — so a bucket’s tags read back empty (completing #447). Kinesis stored the `KeyId` given to `StartStreamEncryption` and echoed it from no describe path. RDS never parsed, stored or serialised `ServerlessV2ScalingConfiguration` (on create *or* modify, so an existing cluster could not be repaired by an update either) and hardcoded `PerformanceInsightsRetentionPeriod` to 7 before dropping it from the response. Contributed by @jbschooley.
