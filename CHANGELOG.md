...
### Added
- **API Gateway v1 — custom domain names route on the data plane** — `CreateDomainName` / `CreateBasePathMapping` already stored mappings, but requests whose `Host` was a registered custom domain never consulted them and often fell through to S3 virtual-host routing (`NoSuchBucket`). The execute-api tier now resolves registered domains with AWS mapping rules (longest matching base path, `(none)` catch-all, strip prefix, stage from the mapping) and dispatches through the existing REST invoke path. TLS/DNS and API Gateway v2 DomainName/ApiMapping remain out of scope. Reported by @liammizrahi (#1030).
