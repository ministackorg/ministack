...
### Changed
- **Lambda — the warm-worker pool is now bounded** — a warm worker is an interpreter subprocess, and nothing ever shrank the pool: workers lived until `DeleteFunction` or `/_ministack/reset`, so one stayed resident per function ever invoked (measured ~14 MB each; a test shard reached 70+). Workers idle for 5 minutes are now released, matching the container pool, and a cold start releases the least-recently-used warm workers beyond 16 live ones. Only the subprocess and its tmpdir are reclaimed, so the next invoke cold-starts as it does after a redeploy. Terminated workers are also waited on rather than left as zombies until GC.
