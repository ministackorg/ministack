...
### Fixed
- **EventBridge Pipes — a DynamoDB stream reaches a Step Functions target** — the poller delivered to SNS and to nothing else, so a pipe whose target was a state machine reported `RUNNING`, advanced no position and moved no records, with no error and no log line: the CDK `DynamoDbSource` → `StateMachineTarget` pipe every stream-driven workflow uses simply never fired. A `states` target now gets one `StartExecution` per batch, carrying the records as the JSON array `$states.input` reads, which is the shape Pipes delivers on AWS. A pipe's stream position advances only once the whole batch has reached its target, so a `StartExecution` that fails and a target this poller cannot serve (SQS, Lambda, Kinesis, …) both leave the records on the stream for the next poll to retry, and log a warning naming the pipe.
