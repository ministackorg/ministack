...
- **S3 — server-side encryption is stated, validated, and enforced instead of silently dropped** — SSE-S3, SSE-KMS and SSE-C headers all returned 200 and were then forgotten: nothing was echoed on the PUT reply or on later reads, an object written under a customer key read back without one, and incoherent requests (a wrong key MD5, key material without the algorithm, SSE-C combined with KMS) were accepted. MiniStack still does not encrypt at rest — like the ACL plane, SSE is now contract state: validated on write (`InvalidArgument` on a bad algorithm, a non-256-bit key, a missing or mismatched key MD5, mutually exclusive modes together, or `aws:kms` naming no key — there is no implicit account `aws/s3` key to fall back to), persisted with the object so HEAD/GET echo it, and enforced on reads for SSE-C, whose whole point is that the object is unreadable without the key: a keyless read (or a key offered for a plain object) is a 400 and a wrong-key read is `403 AccessDenied`, as AWS answers. The state rides `preserved_headers`, so it follows versions, copies and multipart completes through existing plumbing, and the customer key itself is never stored — only its MD5, which is all AWS keeps visible. A bucket's default encryption now applies to the object rather than only stamping the PUT reply. Multipart uploads echo their encryption on initiate, require the initiating SSE-C key on every part (and on UploadPartCopy) and once more on the completion — which is how AWS behaves and what ceph/s3-tests pins — and complete into an object carrying it. Copies never inherit the source's encryption: the destination's SSE comes from the copy request or the destination bucket's default, an SSE-C source demands its key as `x-amz-copy-source-…` parameters, and every response echoes what applied. The POST form's SSE fields work like the headers. Contributed by @gaul.
