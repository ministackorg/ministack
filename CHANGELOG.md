...
- **IoT Core — `RegisterCertificateWithoutCA` and the legacy principal-policy family** — `RegisterCertificateWithoutCA` (`POST /certificate/register-no-ca`) shares the `RegisterCertificate` store, and a duplicate PEM answers `ResourceAlreadyExistsException` (409) with `resourceId`/`resourceArn` from either variant. The deprecated `AttachPrincipalPolicy` / `DetachPrincipalPolicy` / `ListPrincipalPolicies` / `ListPolicyPrincipals` share the modern policy-target store. Contributed by @iot-rocket.
