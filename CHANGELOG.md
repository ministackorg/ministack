...
- **API Gateway (REST) — authorizer results are cached per method ARN and stage** — 1.4.16 cached the allow/deny decision under `(api, authorizer, token)`, so an Allow for `GET /a` replayed on `GET /b` and across stages; the policy document is cached instead and re-evaluated per request. Also fixed: a bad `identityValidationExpression` or TTL raised out of the handler, a missing `principalId` or `policyDocument` answered 403 instead of the 500 configuration error, and the cache never evicted. Contributed by @iot-rocket.
