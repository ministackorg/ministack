...
### Fixed
- **Step Functions — an unimplemented optimized service integration now fails the execution instead of silently succeeding** — a Task using an `arn:aws:states:::<service>:<action>` integration MiniStack does not implement fell through to echoing its input back as a `SUCCEEDED` result, so a state machine whose side effect never ran still reported success. Such a task now fails with `States.Runtime` naming the unimplemented resource; implemented integrations and the generic `aws-sdk:` form are unaffected. Reported by @iwasakar.
- **CloudFormation — an update that would replace a custom-named resource is refused instead of silently executing and destroying data** — a stack update requiring replacement of a resource with an explicit physical name (for example changing a DynamoDB key attribute's type on a table that sets `TableName`) executed the replacement, reported `UPDATE_COMPLETE`, and dropped the table's rows. Such an update now fails and rolls back to `UPDATE_ROLLBACK_COMPLETE` with the message real CloudFormation uses (`CloudFormation cannot update a stack when a custom-named resource requires replacing. Rename <name> and update the stack again.`), leaving the resource and its data untouched; renaming the resource is still the way to force the replacement. Reported by @iot-rocket.
- **CloudFormation — change sets report a valid status, fail when their execution fails, and do not outlive their stack** — `ExecuteChangeSet` wrote `EXECUTE_COMPLETE` into the change set's `Status` (a ChangeSetStatus, whose values are only `CREATE_*`/`DELETE_*`/`FAILED`) and did so at schedule time, so the CDK rejected the change set as not ready (it gates on `Status == CREATE_COMPLETE`) and a failed execution never surfaced. `Status` now stays `CREATE_COMPLETE`, and `ExecutionStatus` moves to `EXECUTE_COMPLETE` or `EXECUTE_FAILED` based on the deployment outcome. Deleting a stack now removes its change sets, and a duplicate change-set name is refused with `AlreadyExistsException`, so a leftover executed change set no longer shadows a same-named one on a re-created stack. A change set with no changes now ends `FAILED` with the reason real CloudFormation gives (`The submitted information didn't contain changes. Submit different information to create a change set.`) instead of reporting `CREATE_COMPLETE`, and executing a change set deletes the stack's other now-invalid change sets. A missing change set now returns the AWS `ChangeSetNotFound` error (404) instead of a non-AWS `ChangeSetNotFoundException` (400), and a direct `UpdateStack` marks the stack's pending change sets `OBSOLETE`. Reported by @iot-rocket.
