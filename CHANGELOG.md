...
- **IoT Core — device shadows over MQTT** — the HTTP shadow APIs worked, but publishing to the reserved shadow topics did nothing: a hard blocker, since device SDKs update shadows exclusively over MQTT. A publish to `$aws/things/<thing>/shadow/get|update|delete` — classic or named — now drives the same shadow store as the HTTP data plane and publishes the `accepted`, `rejected`, `update/delta` and `update/documents` responses back through the broker. Contributed by @iot-rocket.
