...
### Fixed
- **Cognito — PreTokenGeneration's `event.request` now carries `clientMetadata`** — `InitiateAuth`/`AdminInitiateAuth`/`GetTokensFromRefreshToken`'s `ClientMetadata` was never forwarded into the PreTokenGeneration trigger's request, so a Lambda that reads `event.request.clientMetadata` (a common way to pass e.g. a tenant/org id through a refresh call) always saw it as empty, even though the real AWS Cognito Developer Guide documents `clientMetadata` as part of that request shape. `USER_PASSWORD_AUTH`, `ADMIN_USER_PASSWORD_AUTH`/`ADMIN_NO_SRP_AUTH`, `REFRESH_TOKEN_AUTH` (both `InitiateAuth` and `AdminInitiateAuth`), and `GetTokensFromRefreshToken` now thread the caller's `ClientMetadata` through to the trigger. Contributed by @ryan-bennett.
