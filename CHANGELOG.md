...
### Fixed
- **CloudFormation — a rolled-back update leaves the stack reporting what it ran before** — after `UPDATE_ROLLBACK_COMPLETE`, `GetTemplate` returned the template that failed and `DescribeStacks` the failed update's parameters and tags, so a `cdk diff` after a failed deploy compared against the wrong base. The rollback now restores the previous template body, parameters and tags together with the resources and outputs. Contributed by @iot-rocket.
- **CloudFormation — an export counts as in use only when a stack imports it** — `DeleteStack` searched the serialized templates of every other stack for the export name, so a stack that merely carried the name in a string value blocked the delete, while an import through a resolved `Fn::ImportValue` argument (`Fn::Sub` with a parameter) did not. The check now walks the other stacks' `Fn::ImportValue` uses and resolves their arguments against those stacks' own parameters, as CloudFormation does. Contributed by @iot-rocket.
- **CloudFormation — `UpdateStack` without changes is refused** — an identical `TemplateBody`, or `UsePreviousTemplate` with unchanged parameters, ran an empty update (`UPDATE_IN_PROGRESS` / `UPDATE_COMPLETE` events) where CloudFormation answers `ValidationError: No updates are to be performed.`, so a "deploy until nothing changes" loop never ended. The request is refused when the template, every resolved parameter and the tags (when sent) equal what the stack runs; a changed parameter value or tag set is still an update. Contributed by @iot-rocket.
...
### Added
- **CloudFormation — `AWS::IoT::ThingGroup` provisions** — a stack with a thing group rolled back with `Unsupported resource type` while the thing-group API worked, so a fleet modelled for jobs could not deploy. The type now provisions through `CreateThingGroup` (name generated when `ThingGroupName` is omitted, `ThingGroupProperties` and `ParentGroupName` applied, the parent reported under `thingGroupMetadata` by `DescribeThingGroup`), `Ref` returns the thing group id and `Fn::GetAtt` serves `Arn` and `Id`, as the resource reference documents. `ThingGroupProperties` updates in place through `UpdateThingGroup` (a dropped description or attribute set is cleared); `ThingGroupName` and `ParentGroupName` require replacement — the new group is created before the old one is removed, and a parent change under an unchanged custom name gets CloudFormation's own refusal. `QueryString` (dynamic groups) and `Tags` are accepted without effect: the service has neither. Contributed by @iot-rocket.
...
- **S3 — Multi-Region Access Points deploy and serve** — `AWS::S3::MultiRegionAccessPoint` rolled the stack back with `Unsupported resource type` and the MRAP hostname resolved nowhere, so a CDK app fronting regional buckets with one could not deploy. The type now provisions (minting the `.mrap`-suffixed alias, the shape S3 uses, and remembering its member buckets) and `<alias>.accesspoint.s3-global.amazonaws.com` resolves onto the existing virtual-hosted S3 path, so path-style rewriting, IAM enforcement and the handler apply unchanged. The member whose bucket region matches the request region is served, else the first. State is account-scoped, cleared by `/_ministack/reset` and persisted. SigV4A, which S3 requires for an MRAP, is not verified — consistent with the documented no-SigV4 stance: such a request has its presign parameters dropped and is served rather than refused for a signature that could never have matched, and because a SigV4A credential scope carries no region it falls back to the first member. The s3control control plane (`CreateMultiRegionAccessPoint` and the async request token, Get/List/Delete, policies) is not included. Contributed by @mm-salesqueze.
- **CloudFormation — the Cognito resources update in place** — a stack update that changed an `AWS::Cognito::UserPool`, `UserPoolClient`, `IdentityPool` or `UserPoolGroup` fell through to the type's create handler, and for the three with a generated id that meant a second resource under a new id: `Ref` flipped to the new one, the old one was orphaned together with its data (the users in a pool, the role mapping of an identity pool) and nothing in the stack events said so. The four now update the existing record through the service's own path (`UpdateUserPool`, `UpdateUserPoolClient`, `UpdateIdentityPool`, `AddCustomAttributes` for a grown `Schema`; a group's `Description`, `Precedence` and `RoleArn` directly, since the service has no `UpdateGroup`), keeping the physical id and everything stored behind it, and a property the new template drops reverts to the default `CreateUserPool` / `CreateUserPoolClient` fills in, as the resource reference says Cognito does. The pool and the app client are also created through those two calls now, so the record carries every property the update can change (`DeletionProtection`, `EmailConfiguration`, `AccountRecoverySetting`, token validity, `PreventUserExistenceErrors`, …) instead of the handful the create handler copied, and a replaced client carries its whole template over. The properties the resource references mark *Replacement* replace: a `UserPoolClient` whose `UserPoolId` or `GenerateSecret` changed, and a renamed or moved group, are created before the old one is removed and `Ref` follows the new id, while a move to another pool under an unchanged `GroupName` gets CloudFormation's own refusal for a custom-named resource. `AliasAttributes`, `UsernameAttributes` and `UsernameConfiguration` are marked *No interruption* on the reference, but the first two are fixed at `CreateUserPool` and the reference itself calls the third immutable once set, so a change fails the update with a message naming the property rather than altering the record. A group declared without `Precedence` gets none, the null the reference documents as the default, instead of 0; `Fn::GetAtt` gains `UserPoolId` on the pool, `ClientId` on the client and `Name` on the identity pool. Contributed by @iot-rocket.
- **CloudFormation — `EnabledMfas` applies on an `AWS::Cognito::UserPool` update** — the property is *No interruption* on the resource reference and creating a pool with `SOFTWARE_TOKEN_MFA` in it already switched the software-token block on, but an update that added or removed the entry left `GetUserPoolMfaConfig` where it was, so a CDK `mfaSecondFactor` change never reached the emulated pool. The update now goes through `SetUserPoolMfaConfig`: the entry arriving switches the block on, its removal or the property being dropped switches it off, and a pool that never listed it is left without the block. Contributed by @iot-rocket.
- **CloudFormation — `AWS::SecretsManager::Secret` updates in place** — a stack update that changed a secret fell through to the create handler, which minted a second secret under a new ARN: `Ref` and `GetAtt Arn` flipped to it, the original was orphaned together with its versions, and nothing in the stack events said so. The secret now updates through `UpdateSecret` under the same ARN: a changed `SecretString` or `GenerateSecretString` publishes a new `AWSCURRENT` version and the previous one stays behind as `AWSPREVIOUS`; `Description` and `KmsKeyId` are sent only when the template declares or drops them, so a value set outside the stack on a property the template never declared is left alone; `ReplicaRegions` is applied on create and update through `ReplicateSecretToRegions`, and deleting the stack purges the replicas with the secret. `Name` requires replacement and follows the convention already in place: the renamed secret is created before the old one is deleted. Known limits: `Ref` still returns the secret name where AWS returns the ARN (set by the create handler, unchanged here; `GetAtt Arn` carries the ARN, which the reference documents as `Id`), `Type` is not stored by the Secrets Manager service and is ignored, and a region dropped from `ReplicaRegions` keeps its replica because the service has no `RemoveRegionsFromReplication`. Contributed by @iot-rocket.
- **CloudFormation — `AWS::Events::Rule` updates in place** — a stack update that changed a rule fell through to the create handler, which rebuilt the rule record and its target list from the template alone: a target added with `PutTargets` outside the stack was gone, and a target the template had dropped stayed. The rule now updates through `PutRule`, `PutTargets` and `RemoveTargets` under the same name and ARN — the declared targets are reconciled, an undeclared one rides along — and `EventBusName` moves the rule to the other bus under the same name with its targets and tags, the "some interruptions" the resource reference documents. `Tags` are applied on create and reconciled on update through `TagResource` / `UntagResource`, a tag added outside the stack stays, and `Fn::GetAtt` answers the documented `RuleName`. `Name` requires replacement and follows the convention already in place: the renamed rule is created before the old one is deleted. Contributed by @iot-rocket.
- **CloudFormation — `AWS::StepFunctions::StateMachine` updates in place** — a stack update that changed a state machine's definition, substitutions, role, logging or tags fell through to the create handler, which overwrote the record wholesale: the executions and versions behind the ARN were gone, `Tags` were never applied, and nothing in the stack events said so. The machine now updates through `UpdateStateMachine` under the same ARN, keeping its executions; a dropped `LoggingConfiguration` reverts to `OFF`, tags reconcile through `TagResource` / `UntagResource`, and `Fn::GetAtt` serves `StateMachineRevisionId`. `StateMachineName` and `StateMachineType` require replacement and follow the conventions already in place: a renamed machine is created before the old one is deleted, a type change under an unchanged custom name gets CloudFormation's own refusal, and under a generated name the update fails with a message naming the property, since the deterministic name cannot yield a fresh identity. `TracingConfiguration` and `EncryptionConfiguration` are accepted without effect, the store has no field for them. Contributed by @iot-rocket.
- **CloudFormation — `AWS::CloudWatch::Alarm` updates in place** — a stack update that changed an alarm fell through to the create handler, which rewrote the record with a fresh `StateUpdatedTimestamp` on every update, so an alarm that had been in `ALARM` for an hour reported a state change that never happened. The alarm now updates as `PutMetricAlarm` does: the configuration is rewritten under the same name and ARN, a property left out of the template falls back to its default, and the state, its reason, its timestamp and the history stay, as CloudFormation documents. `Tags` are applied on create and update through the alarm's tag set (`PutMetricAlarm` ignores them on an existing alarm). `AlarmName` requires replacement and follows the convention already in place: the renamed alarm is created before the old one is deleted; an alarm deleted behind the stack's back is recreated the same way. `Metrics` (metric math) stays unsupported, as on create. Contributed by @iot-rocket.
- **CloudFormation — `AWS::IAM::Policy` updates in place** — a stack update that changed an inline policy's document, its name or its `Roles` / `Users` / `Groups` fell through to the create handler, which rebuilt the record under a fresh policy id and attached it again, so `GetAtt Id` changed on every edit and `AttachmentCount` drifted from the attachments the entities actually held. Every property of the resource updates with no interruption on AWS, and now it does here: a new `PolicyDocument` becomes the default version through the IAM module's own `CreatePolicyVersion` (pruning at the five-version cap), the entity lists reconcile through attach and detach — the two steps the `AWS::IAM::ManagedPolicy` handler runs, now shared — and a new `PolicyName` re-keys the record under the ARN derived from it, keeping the policy id, the version history and every entity's attachment, with `Ref` following the new name. Deleting either policy type now detaches it from its entities before the record goes, so a role no longer keeps a dangling ARN in its attached list. Contributed by @iot-rocket.
- **CloudFormation — `AWS::ApiGateway::Authorizer` updates in place** — a stack update that changed an authorizer fell through to the create handler: a second authorizer under a new id, `Ref` flipping to it, the old one left on the API. Every property but `RestApiId` is *No interruption* on the resource reference, so the authorizer now updates through `UpdateAuthorizer` under its id; a property the template drops reverts to the value the create fills in (a 300 s result TTL, the `Authorization` header as identity source), `AuthType` is kept as `GetAuthorizer` reports it, and a `RestApiId` change replaces the authorizer in CloudFormation's order. Contributed by @iot-rocket.
- **CloudFormation — `AWS::ApiGateway::Deployment` updates in place** — a deployment whose `Description`, `StageName` or `StageDescription` changed fell through to the create handler: a second deployment under a new id, `Ref` flipping to it, the first left on the API. The three properties are *No interruption* on the resource reference, so the deployment keeps its id: the description patches it, a changed stage name or stage description deploys the same deployment to that stage (a stage the template stops naming stays, as on AWS), and only `RestApiId` and `DeploymentCanarySettings`, which require replacement, create a new one. `StageDescription` is read as the object the reference defines (`Description`, `Variables`), and a failed `CreateDeployment` fails the stack instead of recording an empty id. Contributed by @iot-rocket.
- **CloudFormation — `AWS::KMS::Alias` updates in place** — a stack update that changed an alias fell through to the create handler, which overwrote the record under the same name, and a renamed alias left the old name behind. `TargetKeyId` is *No interruption* on the resource reference, so a changed target re-points the alias under its name as `UpdateAlias` does; `AliasName` requires replacement and the new alias is created before the engine removes the old one. The create now checks the two constraints the reference states — the `alias/` prefix and the reserved `alias/aws/` prefix — and requires `TargetKeyId`. Contributed by @iot-rocket.
...
- **CloudWatch — `PutMetricAlarm` keeps the state timestamp of an existing alarm** — re-putting an alarm stamped a fresh `StateUpdatedTimestamp` on every call, so an alarm that had been in `ALARM` for an hour reported a state change that never happened whenever its configuration was touched. The state, its reason and its timestamp now stay across the re-put and only `AlarmConfigurationUpdatedTimestamp` moves, as the API reference documents ("its state is left unchanged, but the update completely overwrites the previous configuration of the alarm"); both request encodings. Contributed by @iot-rocket.
- **CloudFormation — `AWS::Lambda::Permission` is replaced on update and removed on delete** — a stack update that changed a permission fell through to the create handler and appended a second statement under the same Sid, so the function policy carried both the old and the new principal; a permission declared without an `Id` was never removed at all, because the delete looked for an empty Sid where the create had used the logical id; and `EventSourceToken`, `FunctionUrlAuthType`, `InvokedViaFunctionUrl`, `PrincipalOrgID` and `SourceAccount` were dropped on the way to the statement. Every property of the type requires replacement, so an update now removes the old statement before adding the new one, the delete resolves the Sid exactly as the create does, and create and update go through the Lambda service's AddPermission path, which now keeps all nine documented properties as statement conditions. Contributed by @iot-rocket.
### Fixed
...
### Fixed
- **CloudFormation — stack updates stop destroying resources** — a changed property on a type without an update handler fell through to a re-create: generated-identity types (Cognito pools and clients, Secrets Manager secrets) came back empty under a new id with the old resource orphaned, name-keyed ones wiped their data (Kinesis records, ECR images, group members), and others failed the stack outright (any Route53 record edit, any EventBus property change). Twelve types now update in place — `AWS::Cognito::UserPool`/`UserPoolClient`/`IdentityPool`/`UserPoolGroup`, `AWS::SecretsManager::Secret` (a changed value becomes the new `AWSCURRENT`, same ARN, history kept), `AWS::Kinesis::Stream`, `AWS::ECR::Repository`, `AWS::DynamoDB::GlobalTable`, `AWS::Events::EventBus`, `AWS::CodeBuild::Project`, `AWS::Route53::RecordSet` and `AWS::Scheduler::Schedule` — and the engine gains real replacement semantics: when an update returns a new physical id, the predecessor is deleted in a cleanup phase, as CloudFormation does after `UPDATE_COMPLETE`.
