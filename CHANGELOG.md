...
### Removed
- **Step Functions — `$values` is no longer callable from a JSONata expression** — it is not a JSONata function and not one of the six Step Functions adds (`$partition`, `$range`, `$hash`, `$random`, `$uuid`, `$parse`), so real Step Functions answers `T1006: Attempted to invoke a non-function` for it. MiniStack answered `[1, 2]`, so an expression written and tested here failed the moment the state machine ran on AWS. `$each(obj, function($v) { $v })` returns the same array and `$spread(obj)` returns it as single-key objects; both are JSONata and both run on AWS, and both cover the "iterating over dynamic objects" the request in #669 asked `$values` for.

### Fixed
- **Step Functions — JSONata expressions run on a real JSONata interpreter** — `{% … %}` bodies were evaluated by an ad-hoc operator splitter covering a thin slice of the language, and anything outside that slice either raised `States.QueryEvaluationError` or, worse, answered a wrong value the execution then carried forward. A predicate filter (`$states.input.items[status = 'OPEN']`), a lambda handed to `$map`, `$filter` or `$reduce`, a range predicate (`arr[[0..1]]`), a variable binding (`($x := 2; $x * 3)`), descendant search (`**`), grouping (`items{status: n}`) and much of the function library were all rejected; mapping a field over an array — `$states.input.items.n`, the most ordinary path there is — answered `null` instead of `[3, 9]`, so a Choice compared against nothing and an `Assign` stored nothing, with no error anywhere. A state machine that runs on AWS with `QueryLanguage: JSONata` therefore failed here, or quietly produced different data, for reasons its author could not reproduce against the real service. Expressions now go through the JSONata 2.0.6 interpreter in `ministack/core/jsonata/`, which passes 1674 of the official conformance suite's 1686 cases and adds no runtime dependency. The 12 it skips exercise `$eval`, the one function of the specification Step Functions does not offer. Every variable an `Assign` wrote is bound as a bare `$name`. Three failures are new and deliberate, and all three are `States.QueryEvaluationError`, which a state can `Retry` and `Catch`: an expression that selects nothing fails the execution instead of writing a `null` nobody asked for, which is what AWS does; an expression whose result JSON cannot represent (a function value, `1/0`, `0/0`) fails instead of handing the caller a document no JSON parser accepts; and an expression nested deeper than 200 levels fails instead of exhausting the worker's Python stack. A JSONata error carries its specification code (`T0410`, `D3137`, …) into the cause, so the failure names the rule it broke. Reported in #1519.

- **Step Functions — Map, Parallel, Wait and Fail evaluate their JSONata fields** — only Pass, Task and Choice ever ran a `{% … %}` body, and the other four states dropped theirs without saying so. A Map's `Items` (the example on the AWS documentation page itself) went unevaluated, a Map's or a Parallel's `Output` was silently discarded and the raw result returned in its place, a state-level `Assign` on either one stored nothing, a Wait whose `Seconds` was an expression crashed the execution with a Python traceback, and a Fail took its `Error` and `Cause` as the literal text `{% … %}`. Alongside that, three bindings were wrong: `$states.context` handed the workflow MiniStack's own control state, including every variable an `Assign` had written, where AWS exposes only `Execution`, `State`, `StateMachine`, `Task` and `Map`; `$states.result` and `$states.errorOutput` read back as `null` in the states where AWS makes them unavailable, so `$exists($states.result)` answered `true` on a Pass; and a variable assigned inside a Parallel branch stayed readable after the state completed, where AWS takes it out of scope.
