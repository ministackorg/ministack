...
### Fixed
- **S3 — server-side encryption is stated, validated, and enforced instead of silently dropped** — SSE-S3, SSE-KMS and SSE-C headers all returned 200 and were then forgotten: nothing was echoed on the PUT reply or on later reads, an object written under a customer key read back without one, and incoherent requests (a wrong key MD5, key material without the algorithm, SSE-C combined with KMS) were accepted. MiniStack still does not encrypt at rest — like the ACL plane, SSE is now contract state: validated on write, persisted with the object so HEAD/GET echo it, and enforced on reads for SSE-C (a keyless or plain-object read is a 400 `InvalidRequest`, a wrong-key read a 403 `AccessDenied`, as AWS answers). Only the key's MD5 is stored, and the state follows versions, copies and multipart completes; a bucket's default encryption now applies to the object rather than only stamping the PUT reply, and copies take the destination's SSE from the request or the destination bucket, never the source. Contributed by @gaul.
- **DynamoDB — key attribute types are enforced on `PutItem`, `Query`, and `UpdateTable`** — a key declared `S` accepted an `N` value on `PutItem` (stored as `N`) and on a `Query` key condition (returned the row), and `UpdateTable` carrying only `AttributeDefinitions` silently changed a key's type in place — while `GetItem` / `UpdateItem` / `BatchWriteItem` / `TransactWriteItems` already rejected the mismatch. `PutItem` now returns `ValidationException` "Type mismatch for key <name> expected: S actual: N", a `Query` key-condition type mismatch returns "Condition parameter type does not match schema type", and an attribute-definitions-only `UpdateTable` is rejected ("At least one of ... is required"), so no API changes a key's type. Reported by @iot-rocket.
- **CloudWatch — extended-statistic percentiles are computed instead of aliased to `Average`** — `GetMetricData` and alarm evaluation returned `Average` for any `pNN` extended statistic regardless of the percentile requested (`_stat_value` had a stub, and alarm evaluation read only `Statistic`, never `ExtendedStatistic`). Percentiles are now computed by linear interpolation over the period's samples on both paths, and a percentile alarm's `StateReason` reports the actual statistic (e.g. `p95`) instead of `Average`. Contributed by @MGSousa.
- **S3 — presigned SigV4 URLs verify for virtual-hosted addressing and temporary credentials** — two bugs made valid presigned `GET`/`PUT` URLs fail `403 SignatureDoesNotMatch`: a virtual-hosted (`{bucket}.host`) URL was rewritten to path-style before the signature was recomputed, so it no longer matched the bucket-less URI the client signed; and a URL signed with STS temporary credentials was always verified against the static server secret, never the unique secret STS issued. Verification now runs against the original signed URI, and STS records each issued secret so the verifier resolves the one a credential actually signed with (long-term keys still fall back to `AWS_SECRET_ACCESS_KEY`). Tampered and wrong-secret URLs are still rejected. Reported by @mayankgupta57.
- **Step Functions — `arn:aws:states:::events:putEvents` actually publishes the event** — the optimized EventBridge integration fell through to the task passthrough, so the state reported `SUCCEEDED` while nothing reached any rule target. It now calls EventBridge `PutEvents` and returns its response (`Entries` / `FailedEntryCount`). Reported by @iwasakar.
- **S3 — the SSE-C checks and echoes still missing after the #1444 restoration** — #1444 brought back most of the SSE wiring that #1439's merge resolution lost, but four call sites stayed unwired, all on paths no test pinned: `UploadPartCopy` accepted a part for an SSE-C upload without the create-time key and read an SSE-C copy source without the source key, and neither the `UploadPartCopy` nor the `CompleteMultipartUpload` response echoed the stored algorithm. All three now behave as #1439 originally wrote them. Contributed by @iot-rocket.
- **Aurora DSQL — `SELECT ... FOR UPDATE` is no longer refused for its predicate** — strict mode enforced a rule Aurora DSQL does not (any more): a locking read had to be a single table with an equality on every primary key column, or it was rejected with `0A000` "locking clause such as FOR UPDATE can be applied only on tables with equality predicates on the key". The check compared the WHERE clause's column against the key name as raw SQL text, so `WHERE "id" = $1` did not match key column `id` — and every mainstream ORM (Drizzle, Prisma, TypeORM, SQLAlchemy) quotes its identifiers, so in practice every locking read from an ORM failed, with a message that blamed the user's schema. Measured against a live cluster (eu-central-1, 2026-08-18, `server_version` 16.15), DSQL restricts the lock *strength*, not the predicate: `FOR UPDATE` locks whatever the query selects — no `WHERE` clause, a non-key column, an inequality, `IN`/`OR`, a join, or a table with no primary key all succeed, as do `NOWAIT`, `SKIP LOCKED` and `OF` — while `FOR NO KEY UPDATE`, `FOR SHARE` and `FOR KEY SHARE` are refused with `0A000` "locking clauses other than FOR UPDATE/FOR KEY SHARE are not supported", inside a CTE or subquery included. The predicate rule is gone and the lock-strength rule takes its place, so the emulator stops failing correct code and starts failing the code DSQL actually fails. The new rule is token-based (`FOR SHARE` inside a string literal is text, not a clause) and needs no catalog probe, so a locking read no longer costs a round-trip before it is forwarded.
- **Aurora DSQL — quoted identifiers are normalized the way the server stores them** — `ALTER TABLE ... DROP COLUMN` compared the column against the primary key case-insensitively after stripping quotes, so `DROP COLUMN "ID"` was read as key column `id` and refused, while the primary-key probe dropped the quotes from its relation name — `"myschema".t` became `myschema".t` and resolved to nothing, and a table created as `"MyTable"` case-folded to `mytable` and was not found, silently disabling the rule. Identifiers are now folded as PostgreSQL folds them (bare to lower case, quoted verbatim, `""` unescaped) and the relation is requoted part by part before the lookup, so schema-qualified and mixed-case names resolve.
...
- **S3 — `CRC64NVME` checksums are computed instead of refused** — CRC-64/NVME is the algorithm current AWS SDKs and the CLI checksum uploads with by default, and MiniStack answered it with `InvalidRequest` ("requires optional native dependencies"), so a stock `aws s3 cp` or `aws s3api put-object` failed before it began unless the caller knew to set `AWS_REQUEST_CHECKSUM_CALCULATION=when_required`. The algorithm is plain arithmetic — the reflected form of polynomial `0xAD93D23594C93659` with all-ones init and xorout — so it is now computed from a byte-at-a-time table in the stdlib, adding no dependency: `PutObject` and `CopyObject` validate a client-supplied `x-amz-checksum-crc64nvme` (`BadDigest` on mismatch) and `GetObject` / `HeadObject` return it under `x-amz-checksum-mode: ENABLED`, typed `FULL_OBJECT`. The tests pin the implementation to the algorithm's published check value (`b"123456789"` → `0xAE8B14860A799888`) and to a bit-at-a-time reference rather than to itself. `CRC32C` still requires the native `google-crc32c` and is still refused. Contributed by @gaul.
- **CloudFormation — auto-generated physical names keep their uniqueness suffix when truncated** — `_physical_name` built the full `{stackName}-{logicalId}-{SUFFIX}` string and only then sliced it to `max_len` from the end, so when a deeply-nested stack's own generated name already exceeded a resource's name cap (e.g. Lambda's 64-char `FunctionName` limit), every resource in that stack truncated to an identical string and collapsed onto one physical resource — every Lambda in the nested stack became a single function, so only one ever ran. The suffix (a deterministic hash of stack name and logical id) that guarantees uniqueness is now preserved: the `{stackName}-{logicalId}-` prefix is truncated and the suffix always kept, matching how AWS and the CDK generate physical names. Contributed by @ryan-bennett.
- **IAM — role `Description` charset is validated** — `CreateRole`, `UpdateRole` and `UpdateRoleDescription` now reject a description outside `[\u0009\u000A\u000D\u0020-\u007E\u00A1-\u00FF]` or longer than 1000 characters with a `400 ValidationError`, matching IAM. Reported by @iot-rocket.
- **CloudFormation — `AWS::SSM::Parameter` goes through the SSM API** — instead of writing the store directly, so a create over an existing name fails (`ParameterAlreadyExists`), updates increment `Version` and record history, a `Name` change replaces the parameter, a `SecureString` `Type` is rejected (as CloudFormation does), and `Fn::GetAtt` exposes `Arn` / `Type` / `Value`. Reported by @iot-rocket.
- **CloudFormation — `AWS::SSM::Parameter::Value<...>` is re-resolved on `UpdateStack`** — the SSM parameter name is kept and re-resolved on every operation, so an update with `UsePreviousValue=true` picks up a value changed in Parameter Store since the last deploy. Reported by @iot-rocket.
