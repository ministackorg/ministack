...
### Fixed
- **CloudWatch — `GetMetricData` extended-statistic percentiles (`pNN`) are computed correctly instead of aliased to `Average`** — `_stat_value` had a stub for extended percentiles that unconditionally returned `Average`, regardless of which percentile was requested: `p50`, `p95`, and `p5` over the identical dataset all came back as the same number, silently. Percentiles are now computed from the raw per-period sample values via linear interpolation between the two nearest ranks (CloudWatch's documented method), so distinct percentiles diverge correctly (e.g. `p95` reflects the high tail, `p5` the low tail, of the same window). The same gap existed one level up in alarm evaluation: a `PutMetricAlarm` configured with `ExtendedStatistic` (as opposed to `Statistic`) had that field ignored entirely and was evaluated against `Average` instead — alarm evaluation now honors `ExtendedStatistic` when it's set.
