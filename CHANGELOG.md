...
### Added
- **CloudWatch Logs — `GetLogRecord`, Insights `@ptr` rows, and `StartLiveTail`** — `StartQuery` / `GetQueryResults` previously stubbed empty results, so there was no way to obtain an Insights `@ptr` or round-trip `GetLogRecord`. `PutLogEvents` now assigns an opaque pointer per event; Insights queries return matching rows in the AWS field/value shape (including `@ptr`, `@timestamp`, `@message`, `@logStream`, `@log`); and `GetLogRecord` resolves those pointers to the full transformed field map (`unmask` accepted, masking not implemented). `StartLiveTail` holds a wire-valid `application/vnd.amazon.eventstream` open until client disconnect (`initial-response`, then `sessionStart`, then `sessionUpdate` frames fed by matching concurrent `PutLogEvents`); idle empty updates use `MINISTACK_LIVE_TAIL_IDLE_SECONDS` (default 1s). `DescribeLogGroups` now returns both `arn` (with trailing `:*`) and `logGroupArn` (StartLiveTail-safe, no star). Full CWLI grammar remains out of scope — simple `fields` / `limit` parsing only. Contributed by @ovsteenb.
