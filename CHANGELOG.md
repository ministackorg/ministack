...
### Added
- **CloudFront — cache policies** — `CreateCachePolicy`, `GetCachePolicy`, `GetCachePolicyConfig`, `UpdateCachePolicy`, `DeleteCachePolicy`, and `ListDistributionsByCachePolicyId` under `/2020-05-31/cache-policy`, so Terraform's `aws_cloudfront_cache_policy` creates, reads, updates, and deletes end-to-end. The full `CachePolicyConfig` round-trips — `MinTTL`/`DefaultTTL`/`MaxTTL` and `ParametersInCacheKeyAndForwardedToOrigin` with the gzip/brotli flags and the header/cookie/query-string behaviors and name lists — with `ETag` on every read, `If-Match` enforced on update/delete (`InvalidIfMatchVersion` / `PreconditionFailed`), and `CachePolicyAlreadyExists` / `NoSuchCachePolicy` / `CachePolicyInUse` errors matching AWS. Reported by @wparad.
