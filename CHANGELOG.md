...
- **Lambda — re-entrant invocations no longer deadlock the worker pool** — handlers run with `AWS_ENDPOINT_URL` pointed back at the emulator, so a handler that calls `Invoke` (or hits an API whose authorizer is a Lambda) needs a second slot on the shared `asyncio.to_thread` pool while its own invocation still holds one; once in-flight invocations reached the pool size (`MINISTACK_WORKER_THREADS`, default 64), nested calls queued behind the very callers waiting on them and timed out together. Handler execution now runs on a dedicated per-invocation thread instead of the bounded default executor, on every path that can reach a handler (direct invoke, API Gateway v1/v2, ALB, Cognito triggers, AppSync authorizers, CFN custom resources); an AST guard test keeps future invoke call sites off the shared pool. Contributed by @iot-rocket.
