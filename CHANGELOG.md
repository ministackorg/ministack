...
### Added
- **Amazon Location — trackers and device positions** — `CreateTracker`, `DescribeTracker`, `UpdateTracker`, `ListTrackers`, `DeleteTracker`, the four device-position operations and `AWS::Location::Tracker`. Positions are held in memory, newest 100 per device; `PositionFiltering` applies. Contributed by @iot-rocket.
- **AWS Signer — `StartSigningJob` with its S3 side effect** — plus `DescribeSigningJob`, `ListSigningJobs`, `PutSigningProfile` and `GetSigningProfile`. Signing is synchronous and synthetic: the marker object lands at `prefix + jobId` before the call returns, so the job is already `Succeeded`. Contributed by @iot-rocket.
- **IoT Wireless — `GetPositionEstimate`** — `POST /position-estimate` answers the raw GeoJSON blob the output shape's `payload` trait declares, not a JSON envelope. The estimate resolves from `Ip` and is deterministic. Contributed by @iot-rocket.
- **Amazon Translate — batch text translation jobs** — `StartTextTranslationJob`, `DescribeTextTranslationJob`, `ListTextTranslationJobs` and `StopTextTranslationJob`. Contributed by @ppettitau.
- **Lambda Core — network connectors** — `CreateNetworkConnector`, `GetNetworkConnector`, `UpdateNetworkConnector`, `DeleteNetworkConnector` and `ListNetworkConnectors` on `/2026-04-04/network-connectors`. The service signs with Lambda's own credential scope, so the surface is routed by path. Reported by @edersonbrilhante.
- **CloudFormation — the `Rules` section is evaluated** — rules run after the parameters resolve and before any resource is touched; a false `Assert` refuses the operation with its `AssertDescription`, so the CDK's `CheckBootstrapVersion` rule refuses an outdated bootstrap instead of passing silently. Contributed by @iot-rocket.
- **CloudFormation — the `AWS::Include` transform** — an embedded `Fn::Transform` naming `AWS::Include` is replaced by the S3 object its `Location` points to before the template is validated. Contributed by @iot-rocket.

### Changed
- **API Gateway — a REST method's `COGNITO_USER_POOLS` authorizer is enforced** — the v1 data plane matched only `NONE`, `AWS_IAM` and `CUSTOM`, so a method fronted by a user-pool authorizer served every caller. The token is now verified against the pools in `providerARNs` and its claims reach the backend as `requestContext.authorizer.claims`. `PutMethod` also stops discarding `authorizationScopes`. Contributed by @ppettitau.
- **CloudFormation — the template and stack quotas are enforced** — a template of any size or shape was accepted, so a template a real account refuses deployed. Body, resource, parameter, output and mapping limits are now checked before a stack record exists, and `ValidateTemplate` takes `TemplateURL`. Contributed by @iot-rocket.
- **CloudFormation — `Capabilities` are enforced under `AUTH=true`** — a template with IAM resources or a `Transform` now answers `InsufficientCapabilitiesException` and creates nothing. Without `AUTH` nothing changes. Contributed by @iot-rocket.

### Fixed
- **EC2 — `AvailabilityZoneId` is populated on subnets** — `DescribeSubnets` and `CreateSubnet` always returned `null`, and a consumer that recomputes it when missing could crash on that path. The id is derived from the zone, restored state is backfilled, and the two members always agree. Contributed by @Alexis-DevOps.
- **EC2 — `DescribeAvailabilityZones` reports `ZoneType`** — the field was omitted, so a consumer that branches on it read it as empty. Every zone reports `availability-zone`. Contributed by @Alexis-DevOps.
- **EC2 — `CreateFleet` applies a launch template's instance tags** — instances came back with an empty `Tags` list even though the template carried `TagSpecifications`. Reported by @edersonbrilhante.
- **Step Functions — numeric `*Path` Choice comparisons** — `NumericLessThanPath`, `NumericGreaterThanPath`, `NumericLessThanEqualsPath` and `NumericGreaterThanEqualsPath` evaluated false, so a loop guarded by one never exited. Contributed by @jayjanssen.
- **SES — `SendBulkEmail` is routed** — `POST /v2/email/outbound-bulk-emails` reached no handler. Contributed by @jgrumboe.
- **Glue — the Data Catalog operations** — catalog create, read and update.
- **AWS Signer — a tag key with digits is accepted** — the tag-key pattern was applied more narrowly than the service models it, refusing keys such as `env1`.
- **IoT Wireless — the error body and default confidence level** — the message member went out lowercase where the shapes declare `Message`, and the reported `horizontalConfidenceLevel` was `0.67` where the service documents `0.68` as the default.
- **Amazon Location — `ValidationException` carries `reason` and `fieldList`** — both are modeled members and were never populated.
- **Amazon Translate — timestamps are whole epoch seconds** — `SubmittedTime` and `EndTime` went out as floats, which a Java or Go SDK v2 client rejects on a `timestamp` member.
- **IoT Wireless, Signer, Amazon Location — error bodies are `application/json`** — all three are rest-json services and were answering the json-protocol content type.

### Internal
- **CI — install with uv and collect test shards in one pass** — the shard planner ran `pytest --collect-only` once per marker filter for counts a single pass already has, and a concurrency group now cancels superseded pull-request runs. No user-visible behavior changes. Contributed by @jgrumboe.
- **Lint — `F401` is enforced** — unused imports were ignored repo-wide pending review; the rule is on and the four dead imports it found are gone. No user-visible behavior changes.
- **Testcontainers — `containerd` bumped to 1.7.35** in the Go example module, for CVE-2026-53495. No user-visible behavior changes.
