...
### Added
- **Amazon Location — trackers and device positions, with `AWS::Location::Tracker`** — the service did not exist at all, so a geotracking workload (a topic-rule Lambda writing `BatchUpdateDevicePosition`, an operations script reading positions back) had to deploy with its tracker resource condition-gated out, its rule disabled, and its IAM statement wildcarded because `!GetAtt Tracker.Arn` could not resolve. `CreateTracker`, `DescribeTracker`, `UpdateTracker`, `ListTrackers` and `DeleteTracker` manage trackers, and `BatchUpdateDevicePosition`, `GetDevicePosition`, `BatchGetDevicePosition` and `GetDevicePositionHistory` serve the device-position data plane — with the modeled wire shapes (`geo` signing scope, iso8601 `timestamp` members, `Position` as the `[lon, lat]` double pair, per-entry `Errors` for malformed or out-of-range batch entries, `GetDevicePositionHistory` defaulting to the last 24 hours) and `ResourceNotFoundException`/`ConflictException` with the live service's wording where the API declares them. Trackers only, and metadata plus an in-memory position store: `PricingPlan`/`KmsKeyId`/`PositionFiltering`/`EventBridgeEnabled`/`Tags` are stored and echoed but drive no behavior (no KMS encryption, no EventBridge position events; real `TimeBased` filtering stores at most one position per 30 seconds per device and retains position data for 30 days — MiniStack stores every sample and bounds per-device history at the newest 100), and there are no tracker consumers, geofences, maps, places, or routes. The CloudFormation resource `Ref`s to the tracker name, exposes `Arn`/`TrackerArn`/`CreateTime`/`UpdateTime`, and updates `Description`/`PositionFiltering`/`EventBridgeEnabled` in place through `UpdateTracker`. Contributed by @iot-rocket.
