...
### Fixed
- **API Gateway v2 — `AWS::ApiGatewayV2::Authorizer` CloudFormation support** — deploying a CDK/CloudFormation stack with a JWT authorizer on an `HttpApi` (e.g. `HttpUserPoolAuthorizer`) previously failed outright with `Unsupported resource type: AWS::ApiGatewayV2::Authorizer` and rolled the whole stack back, even though the same authorizer already worked when created via Terraform's `aws_apigatewayv2_authorizer` or the raw API. A provisioner now maps CFN properties onto the existing authorizer store, and `AWS::ApiGatewayV2::Route`'s `AuthorizerId`/`AuthorizationScopes` (previously dropped) are now carried through so the route is actually enforced. Reported by @ryan-bennett.
