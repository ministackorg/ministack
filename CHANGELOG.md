...
- **RDS `DescribePendingMaintenanceActions` no-op surface** — MiniStack now accepts the operation and returns an empty `PendingMaintenanceActions` list, including calls with a `ResourceIdentifier` and ignored `Filters`, `Marker`, or `MaxRecords`. This unblocks brownfield state capture tools that expect the RDS API surface but do not need modeled maintenance events yet. Contributed by @jayjanssen.
