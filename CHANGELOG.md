...
- **IoT Core — MQTT 5.0 alongside 3.1.1** — the broker ignored the CONNECT protocol level, so an MQTT 5 client got a 3.1.1 CONNACK back and died in its decoder. The level now picks the wire format per connection — property blocks, the four subscription options, reason codes, Session Expiry Interval — and both interoperate; a malformed packet gets reason code 0x81, an unsupported level CONNACK 0x01. Shared subscriptions, topic aliases and message expiry stay out of scope. Contributed by @iot-rocket.
