...
- **API Gateway — REST request validators, API keys, throttling and quotas are enforced** — `CreateRequestValidator`, `GetRequestValidators`, `GetRequestValidator`, `UpdateRequestValidator` and `DeleteRequestValidator` are implemented, a method's `requestValidatorId` is applied to the request body and parameters, and an API key is checked against its usage plan's rate, burst and quota. Reported by @iot-rocket.
- **CloudFormation — update handlers for twelve more types** — a stack update of these types re-ran the create: the four ELBv2 types, `AWS::Route53::HostedZone`, `AWS::WAFv2::WebACL` and `AWS::Backup::BackupPlan` came back under a new id, `AWS::IoT::ThingType` failed with `ResourceAlreadyExistsException`, and certificate, email identity, backup vault and API Gateway account changes were lost. Each type now updates in place where its reference allows, replaces otherwise and refuses to replace a custom-named resource. Also fixed: a stack's backup plan read back without rules, its email identity was invisible to SES v2, `DescribeTags` raised on its target groups, and ACM timestamps answered the time of the call. Contributed by @iot-rocket.
...
- **RDS — PostgreSQL instances serve TLS** — a generated CA signs a server certificate for each container, so a client connecting with `sslmode=verify-full` completes the handshake instead of failing. `GET /_ministack/rds/ca.pem` returns that CA, the local stand-in for AWS's certificate bundle.
...
- **CloudFormation — `DeletionPolicy` defaults to `Retain` for RDS clusters and standalone instances** — both defaulted to `Delete`, so a stack delete destroyed the database. AWS defaults them to `Snapshot`; this emulator takes no snapshots, so it keeps the resource instead of destroying it.
...
- **IoT — the registered-certificate event comes from the device connect** — `RegisterCertificate` published `$aws/events/certificates/registered/{caCertificateId}`, and an mTLS connect with an unknown certificate got CONNACK 5 with nothing created. AWS publishes the event only on the connect: a certificate signed by an ACTIVE CA with auto-registration enabled is created `PENDING_ACTIVATION`, the event carries `sourceIp`, and the connection closes without a CONNACK. The listener now does the same, and `RegisterCertificate` with status `PENDING_ACTIVATION` is `CertificateStateException` 406. Contributed by @iot-rocket.
...
- **IoT Data — HTTPS `Publish` refuses the topics AWS refuses** — any topic starting with `$` was accepted, so a caller could forge `$aws/events/...` lifecycle events and fire the rules on them. AWS answers `InvalidRequestException` 400 "Topic can't start with $" outside the `$aws/rules/`, `$aws/things/`, `$aws/certificates/`, `$aws/provisioning-templates/`, `$aws/device_location/` and `$aws/commands/` families, and "Invalid publish to restricted topic using HTTP" for the jobs, Device Defender and commands topics, which are MQTT-only. MiniStack now returns the same errors; MQTT publishes are unchanged. Contributed by @iot-rocket.
- **CloudFormation — `AWS::ECR::Repository` keeps what the template declares** — scanning and encryption settings were stored in the template's PascalCase and read back empty, lifecycle policy, repository policy and tags were dropped, `PutImage` into the repository failed and a deleted repository left its images and policies behind. The repository is now created, updated in place and deleted through the ECR store. Contributed by @iot-rocket.
- **CloudFormation — a rolled-back replacement keeps the old resource** — an update handler deleted the predecessor as soon as the replacement existed, so when a later resource failed the rollback removed the replacement too and the stack recorded a resource that was gone. The predecessor is now deleted in the cleanup phase after the update succeeds, and a rollback deletes only the replacement, as measured on AWS. Contributed by @iot-rocket.
...
- **S3 — a notification configuration is stored in the form the wire uses** — the raw request XML was echoed back, so a configuration written with the modern tags (`LambdaFunctionConfigurations`, `LambdaFunctionArn`) came back empty for a client expecting the wire names `CloudFunctionConfiguration` and `CloudFunction`, which also left a CloudFormation-declared Lambda notification invisible. Reported by @jin-gizmo.
...
- **Lambda — an AWS-published extension layer resolves** — a template referencing `LambdaInsightsExtension`, the Parameters and Secrets extension, an AppConfig or OpenTelemetry layer or `AWSSDKPandas` failed to deploy because the publisher account is unknown offline. The reference now resolves by layer name with a `CodeSize` of 0; the bytes are not available, so the extension does not run. `AddLayerVersionPermission` also validates `organizationId` and `statementId` against their patterns.
