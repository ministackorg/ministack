...
### Fixed
- **IoT Core — an HTTP shadow update publishes the reserved-topic notifications** — `UpdateThingShadow` over the data-plane REST API stored the state and emitted nothing, so a topic rule on `$aws/things/<t>/shadow/update/accepted` fired for MQTT updates but stayed silent for HTTP ones, and every HTTP-driven test had to publish the envelope by hand. On AWS the accepted/delta/documents messages are state-change notifications the Device Shadow service emits however the update arrived. An accepted HTTP update now publishes all three through the broker (and an HTTP delete its `delete/accepted`), sharing the MQTT bridge's emission code. Deliberately unchanged: an HTTP `GetThingShadow` publishes nothing — `get/accepted` is the MQTT request's response channel — and HTTP failures publish no `rejected` envelope, the HTTP status already carries them. Sharing that code also fixed two `update/documents` envelope bugs on both transports: the envelope now echoes the triggering request's `clientToken` (AWS includes it when one was used), and its `previous`/`current` members carry `state` + `metadata` + `version` only — the inner `timestamp` they used to copy out of the GET snapshot is not part of the AWS document, the single timestamp sits on the envelope. Contributed by @iot-rocket.
