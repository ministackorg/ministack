...
### Fixed
- **API Gateway (HTTP API / v2) — `REQUEST` authorizer caching is scoped to the route** — the cache stored the verdict under `(apiId, authorizerId, identity)`, so a cached Allow authorized every other route, stage and account it touched. The IAM policy response is now cached and re-evaluated against the request's own route ARN; an invalid response (no `policyDocument`, non-boolean `isAuthorized`) is a `500` and is never cached; a non-numeric TTL no longer raises. Contributed by @iot-rocket.
- **EventBridge — API destination delivery no longer leaks credentials, and the caller-supplied values on the outbound path are validated** — an API destination target makes an outbound HTTP request that carries the connection's credentials to a URL the caller chooses, so each value on that path is now checked before it can reach the wire. (1) Delivery and OAuth token requests no longer follow redirects: a `302` to another host could carry the connection's `Authorization` header to the redirect target (urllib preserves it across a cross-host hop) — a `3xx` is now a non-retryable delivery failure, as on AWS. (2) `CreateApiDestination` / `UpdateApiDestination` reject an `InvocationEndpoint` that is not a dialable `http(s)://` URL, and `CreateConnection` / `UpdateConnection` apply the same check to the OAuth `AuthorizationEndpoint` — the URL a connection's `client_id`/`client_secret` are POSTed to, previously stored unvalidated and failing only later as a log line on a delivery thread. The outbound opener independently refuses non-HTTP(S) schemes, so `file://` never reaches the wire even for a record restored from persistence. This is a scheme guard only — MiniStack is a local emulator where delivering to `127.0.0.1` is the normal case, so loopback, private and link-local hosts stay reachable over http(s) by design. (3) `HttpMethod` and a connection's `AuthorizationType` are validated against their API enums instead of being stored verbatim — an unrecognized method silently became `POST`, and an unrecognized authorization type matched nothing, so the connection delivered with no credentials at all. (4) A connection `ApiKeyName` colliding with a reserved header (e.g. `User-Agent`) can no longer override EventBridge's fixed headers. (5) `DeauthorizeConnection` now removes the connection's authorization parameters, as the API reference says it does, and delivery skips a connection that is not `AUTHORIZED` — previously the credentials stayed in place and every later delivery still presented them. (6) The OAuth token response is read with a 1 MiB bound, the ceiling EventBridge's fixed `Range` header already puts on a delivery response.
