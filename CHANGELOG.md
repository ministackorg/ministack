...
- **EventBridge — event patterns are validated, so a pattern AWS refuses no longer looks fine locally** — `PutRule`, `TestEventPattern`, `CreateArchive` and `UpdateArchive` checked only that a pattern was valid JSON, so a rule that cannot exist on AWS appeared to work here: silence locally where production gives a `400`. All four now raise `InvalidEventPatternException` with the reason the service reports, verbatim — `Empty arrays are not allowed`, `Only one key allowed in match expression`, `Consecutive wildcard characters at pos <n>`, and the rest. `numeric` turns out to be a grammar rather than a list of pairs — one comparison, or a lower bound then an upper — so `[">", 1, ">", 2]` and `["=", 1, "<", 5]` are refused. A malformed `$or` is read as an ordinary field name: EventBridge tries its `$or`-aware compiler, then its pre-`$or` one, and only a pattern both refuse is invalid. Rules restored from persisted state are never revalidated, and the matcher behind that door stays fail-closed: an unacceptable pattern matches nothing rather than raising. Four deliberate or unavoidable divergences: a hex IPv4-mapped IPv6 operand keeps its IPv6 mask width (AWS narrows it to IPv4); a fractional threshold whose literal does not survive conversion to a double is accepted, its text lost to the JSON parse — the integer case *is* refused; a dotted pattern key stays its own segment, not the nested spelling's path; and a validation fault hidden in a duplicated JSON key goes unreported, the parse keeping only the last occurrence.
- **EventBridge — wildcard event-pattern matching now follows AWS semantics** — content-filter `wildcard` used Python `fnmatch`, which treats `?` and `[seq]` as special and handles `\` backwards, so a correctly-escaped guard pattern was silently inert locally, steering rules into a form that blocks nothing in production. Only `*` is special now, `\` escapes (`\*` = literal asterisk, `\\` = literal backslash), and `?`/`[seq]` are literal — matching real AWS `TestEventPattern` on every supported form. The replacement keeps fnmatch's whole-string anchoring and its resistance to catastrophic backtracking: each interior literal compiles to `(?=(?P<gN>.*?lit))(?P=gN)` rather than a plain `.*lit`, so `*/*/*/*/*.json` stays linear against a long non-matching value instead of taking seconds per event on the dispatch path. Compiled patterns are cached, and the state-reset endpoint drops that cache with the rules that wanted it. A run of `*`, and a list operand outside `anything-but`, are now refused at rule creation, as AWS refuses them.
- **EventBridge — `cidr`, `equals-ignore-case`, `exactly` and case-insensitive `prefix`/`suffix` are evaluated instead of matching nothing** — each was accepted into a pattern and then fell off the end of the content-filter dispatch, answering "no match" for every event, so a rule filtering on an IP range or a case-insensitive value silently discarded everything and never fired its targets; a case-insensitive `prefix`/`suffix` was worse still, its object operand raising out of the dispatch and failing the whole `PutEvents` batch with a `500`. `cidr` now matches an IPv4 or IPv6 address against a block, comparing addresses rather than text (IPv6 case and zero-compression are irrelevant) and flooring a block whose host bits are set: `10.0.0.5/24` is the `10.0.0.0/24` block, as AWS does. Only a JSON string can match it — `ipaddress` would read `167772161` as `10.0.0.1` and `true` as `0.0.0.1`, matching a block AWS never matches them against. `equals-ignore-case` accepts either case of *each operand character independently*: not `str.lower()`, which cannot match `straße` against `STRASSE` at all, nor `str.casefold()`, which over-matches by also accepting `strasse`. That relation is asymmetric (operand `ẞ` matches value `ß`, not the reverse) and not even reflexive for a titlecase character, both matching real AWS. `prefix` and `suffix` take a nested `equals-ignore-case`; `anything-but` negates a literal, a list, or a nested `prefix`/`suffix`/`wildcard`/`equals-ignore-case`, singly or as a list — but not `cidr` or `numeric`, which AWS refuses there. One deliberate divergence: an operand character outside the Basic Multilingual Plane matches itself, where AWS walks UTF-16 code units and mangles it into `?`, so its own operand stops matching the value it was written for.
- **EventBridge — `$or` is expanded rather than ignored, and matched every event before** — the pattern walk read only the keys it knew, and an unread key is a matched key, so `$or` went unevaluated: a pattern whose only key was `$or` matched **every** event on the bus and fanned it out to the rule's targets, and one combining `$or` with other keys matched on the siblings alone. It is now read wherever a field name is legal — top level, in `detail`, and in objects nested under it — and expanded at rule creation into one alternative per branch, matching if any does. Expansion writes leaves into the alternatives accumulated so far, so a leaf constrained both inside and outside a `$or` keeps only the **last** value written, in document order: `{"a":["1"],"$or":[{"a":["2"]},{"b":["3"]}]}` matches `a: "2"` and not `a: "1"`, and swapping the two keys reverses both answers. A different leaf merges key by key. AWS's 1000-combination cap is enforced, counted over the `$or` arrays the pattern *text* holds before either compiler runs, so a `detail` field literally named `$or` still counts toward it. One deliberate divergence: a pattern whose `$or`-named fields multiply past that cap is refused here — ten of them, each with two branches, is the first count that trips it — where AWS's fallback compiler, which never reads them as `$or`, may accept it.
- **EventBridge — the whole event envelope is filterable, and an unmodelled key is a path rather than an error** — the walk evaluated `source`, `detail-type`, `detail`, `account`, `region` and `resources` and silently skipped everything else, so a pattern filtering on `id`, `time` or `version` matched every event. AWS has no envelope allow-list: it compiles a pattern into paths, so those three and `replay-name` are ordinary fields and an unrecognized top-level key is just a path the event lacks — `{"nonesuch": ["x"]}` matches nothing, `{"nonesuch": [{"exists": false}]}` matches *everything*. The matcher is now one generic walk over the envelope a target is delivered: `time` matches as the ISO-8601 string the target receives, with no date handling anywhere, as on AWS. `replay-name` is stamped onto replayed events, which AWS's own archive rule — `{"replay-name": [{"exists": false}]}` — needs in order to mean "live traffic only". `resources` now matches on intersection — a rule naming two ARNs matches an event carrying either — where this demanded all of them, by equality only, so a content filter on it never matched. Matching and delivery share one envelope builder, which reads a non-JSON `Detail` as the string leaf it is; such an entry used to match a rule and then raise out of *delivery*, a `500 InternalError` that took every other rule on the bus with it. `InputTransformer` held a third copy that disagreed — `time` as the raw epoch second, a non-JSON `Detail` as `{}` — so `<aws.events.event.json>` is now the event itself and `<aws.events.event>` that same event without `detail`, as on AWS.
- **EventBridge — array values are matched per element, `exists` asks about leaves, and values compare as JSON** — an array was matched by literal intersection, so no content filter reached its elements. AWS flattens arrays recursively and offers each element to the matcher, the field matching if any does; under an array of objects, one element must satisfy a pattern object whole — AWS's array consistency rule. `exists` was answered from the key's presence, but AWS's operators "only work on leaf nodes": an object-valued field, an empty array and an empty object do not exist for it, while a null does — a leaf. Equality was Python's — `True == 1`, both sides coerced to text — so a rule matched values of a type it never named; a string never equals a number now, as on AWS. A null- or object-valued field, or a bare scalar as a pattern value, reached no arm of the dispatch and went unjudged, which reads as matched: every content filter matched a field set to one. Both match nothing now. Still not modeled, tracked as follow-ups: the 4096-character pattern limit, AWS's wildcard-complexity refusal, its 256-leaf rule-size cap, and two flattening corners. `exists: false` beneath an array of objects is answered per element, where AWS asks whether the flattened path exists at all, so it can match where AWS would not. And a path constrained both as a leaf and as an object across a `$or` boundary is treated as unsatisfiable — the right answer except for an event whose array at that path mixes a scalar and an object, and for an object-side `exists: false`, which the path's plain absence satisfies.
