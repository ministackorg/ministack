...
- **IoT Core — `connectivity.*` fleet-index terms** — with `thingConnectivityIndexingMode: STATUS`, `SearchIndex` answers `connectivity.connected`, `.timestamp` (last transition, epoch ms) and `.disconnectReason`, and every hit carries the `connectivity` group — "which of my things are online" now works locally. A thing is connected while an MQTT session using its name as client id is live; the flag is read from the broker's registry, never persisted. Contributed by @iot-rocket.
