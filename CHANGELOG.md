...
- **IAM — a resource-scoped policy matches, instead of denying** — `extract_resource_arn` returned `*` for three whole families of request, and a resource-scoped policy can never match `*`, so under `AUTH=true` the only grant that worked was one written as `Resource: "*"`. First, IoT mapped six path prefixes to ARNs but not `/jobs` or `/provisioning-templates`, and neither data plane had a branch at all: a client signs those with the `iotdata` and `iotjobsdata` scopes, so they arrive as their own service keys and fell through. That rules out what the CDK emits: its `AwsCustomResource` for an IoT job grants `iot:CreateJob` on exactly `job/<jobId>` plus the target `thinggroup/<name>`, the scoping AWS documents, and a CloudFormation stack carrying one failed to deploy here with `is not authorized to perform: iot:CreateJob` while deploying on AWS. A publish and a retained message now resolve to their topic, which carries every level (`topic/sensors/rack-1/temperature`), and a shadow or job execution to its thing. `Publish` also resolves an action again: its URI label is non-greedy, so the SDK percent-encodes the separators and the ASGI server hands them back decoded, and a route compiled as one segment then matched no topic below the first level. Second, the query-protocol services (EC2, CloudFormation, CloudWatch, Auto Scaling, ElastiCache) carry their parameters in a form-encoded body when the SDK POSTs, which botocore does, so nothing reached the query string and every branch reading it resolved nothing: `ec2:AuthorizeSecurityGroupIngress` scoped to one security group was denied for that group. The router already parsed that body to find `Action` and now keeps the rest of it, merged underneath the query string, which still wins. Third, an `execute-api` invoke was authorized against `*` as well, so a policy naming one API and stage, which is what `grantExecute` and every service-to-service grant emit, never matched; it now resolves to `arn:aws:execute-api:<region>:<account>:<api-id>/<stage>/<METHOD>/<path>`, built by the same helper the Lambda authorizer's method ARN uses, and authorized once the stage is known so a v2 API on `$default` is not charged for a stage segment it does not have. Contributed by @iot-rocket.
