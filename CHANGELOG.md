...
### Fixed
- **EC2 — IAM instance profile association APIs implemented** — `AssociateIamInstanceProfile`, `DescribeIamInstanceProfileAssociations`, `DisassociateIamInstanceProfile`, and `ReplaceIamInstanceProfileAssociation` previously failed with `InvalidAction: Unknown EC2 action` or had no persisted association state behind `RunInstances(IamInstanceProfile=...)`. EC2 now stores a real association record, surfaces it through the Query/XML response shapes botocore expects, filters by `association-id` / `instance-id` / `state`, and clears the active association when the instance is terminated. This lets Terraform's `aws_instance.iam_instance_profile` apply, refresh, and subsequent `plan` round-trip without drift.
