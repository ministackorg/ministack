...
### Fixed
- **IAM — a CloudFormation-provisioned policy no longer breaks the read APIs** — `AWS::IAM::Role` with `ManagedPolicyArns`, and both policy resource types, wrote records the IAM API never produced: `AttachedPolicies` held `{PolicyName, PolicyArn}` dicts where every reader expects a bare ARN, so `GetAccountAuthorizationDetails` and `ListAttachedRolePolicies` answered 500 (`unhashable type: 'dict'`) and `ListEntitiesForPolicy` never matched; and `Versions` was a list rather than a dict keyed by version id, so `GetPolicyVersion` and `ListPolicyVersions` failed on any policy a stack created. Policies and attachments now go through the IAM module itself, so one shape reaches every reader and `AttachmentCount` tracks stack-created attachments as it does API ones. `AWS::IAM::ManagedPolicy` also honours its documented `Roles`, `Users` and `Groups` properties, which it previously ignored, and both resource types return the attributes CloudFormation documents — the eight on `ManagedPolicy`, `Id` on `Policy` — instead of answering the physical id for every `Fn::GetAtt`. `Ref` on `AWS::IAM::Policy` returns the policy name, matching AWS. `GetAccountAuthorizationDetails` now also lists an attached policy whose document it cannot read, agreeing with `ListAttachedRolePolicies` rather than silently dropping the row. Reported by @iot-rocket.
