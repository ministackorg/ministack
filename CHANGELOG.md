...
- **AWS Signer — synchronous `StartSigningJob` with the S3 side effect** — the `signer` API did not exist, so code-signing call paths could only run locally behind a disabled-signing flag and shipped to production untested. `StartSigningJob`, `DescribeSigningJob`, `ListSigningJobs`, `PutSigningProfile` and `GetSigningProfile` are now served natively (REST-JSON, URIs per botocore's signer model). Three deliberate divergences, each pinned by a test: signing is synchronous — the source object is validated against the local S3 store and the signed object written to `destination.prefix + jobId` before Start returns, already `Succeeded` (a missing source or destination bucket is `ResourceNotFoundException` at Start with no job recorded); the signed object is a synthetic JSON marker carrying the source reference and the SHA-256 of the source bytes, not a real signature; and an unknown `profileName` is accepted, auto-registering a minimal implicit profile. `source.s3.version` accepts an absent value as "current object" (what `head_object` reports on an unversioned bucket); a literal `"null"` version reads the stored `"null"` version when one exists (suspended-bucket writes, pre-versioning objects) and falls back to the current object otherwise. The signed object always lands at exactly `prefix + jobId` — on real AWS that key is platform-dependent (`AWSIoTDeviceManagement-SHA256-ECDSA` writes exactly that; `AWSLambda-SHA384-ECDSA` appends the source archive extension, e.g. `.zip`; both live-measured). Job and profile state persists when `PERSIST_STATE=1`. Contributed by @iot-rocket.
