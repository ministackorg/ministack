...
### Added
- **Lambda — recursive-loop detection** — `PutFunctionRecursionConfig` stored `RecursiveLoop` but nothing ever read it, so a function that invoked itself asynchronously looped forever. Direct Lambda → Lambda hops from Python and Node handlers now carry a depth counter; past 16 the invocation is dropped before it runs with `RecursiveInvocationException` and a `RecursiveInvocationsDropped` metric, unless `RecursiveLoop` is `Allow`. Contributed by @iot-rocket.
