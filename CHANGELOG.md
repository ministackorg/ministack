...
- **CloudFormation — `AWS::Events::EventBus` survives a stack update** — the type has no update handler, so a stack update falls back to calling create again (documented as relying on create being idempotent, same as SQS/SNS); its create instead unconditionally rejected the bus's own name as "already exists" once it had been created once. A name computed client-side and baked into the template (e.g. CDK's `EventBus` construct, which derives its default name from the construct path) is identical on every deploy, so any stack containing one failed *every* update, even when the bus itself hadn't changed. The bus is now left alone across an update, matching real CloudFormation; a name genuinely colliding with an unrelated bus (or the reserved `default` bus) still fails, as before. Contributed by @ryan-bennett.
