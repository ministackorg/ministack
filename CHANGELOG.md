...
### Fixed
- **ECS — a restored service relaunches its tasks** — `restore_state` marks every restored task `STOPPED`, since its container went with the process that ran it, and nothing reconciled the services afterwards. A restarted MiniStack reported each service’s persisted `runningCount` while no container existed, and a load balancer in front of it kept forwarding to addresses nothing was listening on — the service looked healthy and every request failed, recoverable only with a manual `force-new-deployment`. ACTIVE services are now reconciled once after a restore, on a daemon thread so image pulls do not block startup (`ECS_RESTORE_RECONCILE_DELAY_SECONDS`, default 2). Contributed by @jbschooley.
