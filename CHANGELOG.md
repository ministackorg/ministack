...
### Added
- **API Gateway — registered custom domains route the data plane** — `CreateDomainName` and `CreateBasePathMapping` stored their records but no request ever consulted them: a request addressed by a custom domain host fell through the host-pattern service guessing, typically landing in S3 as `NoSuchBucket` (or in a wrong service when the domain happened to contain a token such as `iot.`). A registered domain now resolves before any host guessing runs — longest base path wins, the `"(none)"` sentinel (or an explicit empty `basePath`) is the root mapping, and a mapping that names a stage is authoritative so no stage segment is expected in the path, matching the `BASE_PATH_MAPPING_ONLY` routing mode the records already default to; a stage-less mapping instead takes the stage from the first remaining path segment, like a plain execute-api URL. The domain lookup ignores host case, and an OPTIONS preflight to a registered domain flows through the API's own `corsConfiguration` rather than the generic wildcard preflight. Unregistered hosts keep the existing fallthrough; the cost is a linear scan over the registered domain names for any dotted-host request that matches no execute-api pattern (a local stack registers a handful of domains at most). Contributed by @iot-rocket.
