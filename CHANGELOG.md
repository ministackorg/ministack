...
- **Tests — two suite-order races made deterministic** — `test_cfn_unnamed_dynamodb_table_survives_unrelated_update` compared the global `ListTables` result across its update window, and `test_glue_batch_stop_job_run` called `BatchStopJobRun` while `StartJobRun`'s background thread was still running. Both now assert only on state they own, so a loaded parallel shard no longer fails them. Contributed by @iot-rocket.
