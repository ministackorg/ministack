...
- **CloudFront — `AWS::CloudFront::CachePolicy` updates in place** — the type had no update handler, so a stack update fell through to the create handler, which is idempotent on the config `Name` and returned the existing policy untouched: `UpdateStack` reported `UPDATE_COMPLETE` while `GetCachePolicy` kept serving the create-time config (a `Comment` measured as `v1` after the template had moved it to `v2`). A changed `Name` did take effect, but as a replacement — a new policy Id, the old one deleted, breaking every `Ref` that had already reached a distribution. The resource reference marks `CachePolicyConfig` and every field under it "Update requires: No interruption", so the policy now keeps its Id across every change, a rename included; the config is re-parsed through the service's own `_parse_cache_policy_config`, so a property the template drops reverts to its create default, and a rename onto a name another policy holds is refused, as `UpdateCachePolicy` refuses it. Contributed by @iot-rocket.
- **CloudFront — `AWS::CloudFront::OriginRequestPolicy` updates in place** — the same create-only gap: without an update handler the stack update landed in the name-idempotent create handler, which returned the policy it had made before, so a changed `Comment`, `QueryStringBehavior` or header whitelist never reached `GetOriginRequestPolicy` (measured: `v1` still served after the template said `v2`), while a changed `Name` replaced the policy under a new Id. `OriginRequestPolicyConfig` and its `Comment`, `CookiesConfig`, `HeadersConfig`, `Name` and `QueryStringsConfig` are all "Update requires: No interruption", so the policy now keeps its Id across every change, a rename included, the config is re-parsed through the service's own parser, dropped properties included, and a rename onto a taken name is refused as the service refuses it. Contributed by @iot-rocket.
- **CloudFront — `AWS::CloudFront::ResponseHeadersPolicy` updates in place** — third of the create-only policy types: a stack update returned the existing policy from the create handler, so a changed `AccessControlMaxAgeSec` stayed at its create value (measured: `86400` after the template had said `600`) and an edited custom header kept the old value, while a rename replaced the policy. `ResponseHeadersPolicyConfig` and its `Comment`, `CorsConfig`, `CustomHeadersConfig`, `Name`, `RemoveHeadersConfig`, `SecurityHeadersConfig` and `ServerTimingHeadersConfig` are all "Update requires: No interruption". The policy now keeps its Id, a rename included, a rename onto a taken name is refused as the service refuses it, and because the config goes back through `_parse_rhp_config` a `CorsConfig` the template drops leaves the policy rather than persisting. Contributed by @iot-rocket.
- **CloudFront — `AWS::CloudFront::OriginAccessControl` updates in place** — the create handler is idempotent on the OAC `Name`, so a stack update that changed `SigningBehavior`, `OriginAccessControlOriginType` or `Description` was discarded and `GetOriginAccessControl` kept serving the create-time config (measured: `never` after the template had said `no-override`), and a signing behaviour that stays `never` leaves the origin unsigned. `OriginAccessControlConfig` and its `Description`, `Name`, `OriginAccessControlOriginType`, `SigningBehavior` and `SigningProtocol` are all "Update requires: No interruption", so the OAC now keeps the Id an origin refers to across every change, a rename included, a rename onto a taken name is refused as the service refuses it, and a dropped `Description` reverts to the same default the create handler applies. Contributed by @iot-rocket.
- **CloudFront — `AWS::CloudFront::Function` updates rather than re-creates** — the type's create handler overwrites the whole function record, so the fall-through did carry a changed `FunctionCode` or `Comment` across, but as a re-create: the function's `CreatedTime` was stamped again on every stack update (measured 11 ms apart across one `UpdateStack`), and any state the record holds beyond what the template carries went with it. `Name` is the type's only "Update requires: Replacement" property, so it now replaces (create the new function, remove the old one; a rename onto a name another function holds is refused instead of writing over it) while `AutoPublish`, `FunctionCode` and `FunctionConfig` update the existing function through the same body the service's `UpdateFunction` writes: the new source lands in DEVELOPMENT, `AutoPublish: true` republishes it to LIVE, and the creation time stays. Contributed by @iot-rocket.
### Fixed
