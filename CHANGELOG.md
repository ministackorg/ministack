...
- **S3 — SSE-C is enforced and echoed on `UploadPartCopy` and `CompleteMultipartUpload`** — `UploadPartCopy` accepted a part for an SSE-C upload without the upload's key and read an SSE-C copy source without the source key, and neither `UploadPartCopy` nor `CompleteMultipartUpload` echoed the stored encryption on its response. `UploadPartCopy` now requires the upload's SSE-C key and the copy source's key (including a `?versionId=`-qualified source) and echoes `SSECustomerAlgorithm` / `SSECustomerKeyMD5`; `CompleteMultipartUpload` echoes `ServerSideEncryption`. Contributed by @iot-rocket.
