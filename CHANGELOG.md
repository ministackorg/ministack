...
### Fixed
- **EC2 — `AuthorizeSecurityGroup{Ingress,Egress}` accept the legacy source-group/CIDR parameters and return `ReferencedGroupInfo`** — a source security group passed via the AWS CLI's `--source-group` (the flat top-level `SourceSecurityGroupId`/`SourceSecurityGroupName` form, also emitted by older SDKs) was silently dropped: the parser only handled the nested `IpPermissions.N.*` form, so the rule never appeared in `DescribeSecurityGroups` (`UserIdGroupPairs` came back empty) or `DescribeSecurityGroupRules`. The flat single-rule form is now parsed alongside the nested form, and the Authorize response now includes a `ReferencedGroupInfo` item (with a `SecurityGroupRuleId` consistent with `DescribeSecurityGroupRules`) for group-pair rules, matching AWS — so Terraform's `aws_security_group_rule` resolves the new rule instead of timing out its create waiter. Reported by @kamegoro.
- **IAM — instance-profile tagging actions implemented** — `TagInstanceProfile`, `UntagInstanceProfile`, and `ListInstanceProfileTags` previously failed with `InvalidAction: Unknown IAM action`. They are now handled, tags are stored on the instance-profile object (including tags supplied at `CreateInstanceProfile` time), and they read back from `GetInstanceProfile` / `ListInstanceProfiles` (via the `Tags` member) and `ListInstanceProfileTags`. This read-back lets Terraform's `aws_iam_instance_profile` settle to "No changes" on re-apply instead of detecting tag drift.
