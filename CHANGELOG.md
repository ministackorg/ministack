...
- **IoT Core — rule SQL gains `newuuid()`, `replace()`, and `clientid()`** — the SELECT/WHERE expression evaluator knew only `topic()`, `timestamp()`, and `encode()`, so every other call resolved to Undefined and its field vanished from the projection without a word. `newuuid()` mints a UUID per evaluation, `replace(<expr>, '<from>', '<to>')` rewrites string values, and `clientid()` reports the publishing MQTT client's id — Undefined for an HTTP `Publish`, as on AWS. Contributed by @iot-rocket.
- **IoT Core — topic rules dispatch `republish`, `dynamoDBv2`, and `sns` actions** — only `lambda` actions ever ran; the other three were stored and then silently did nothing, so a "telemetry → rule → DynamoDB table" pipeline passed rule creation and dropped every event. All three now dispatch in-process on each matching publish, and a failure — a missing table or topic included — is logged, leaves the rule's other actions running, and fires its `errorAction`. Contributed by @iot-rocket.
...
- **IoT Core — topic-rule `WHERE` clauses are evaluated and rule SQL is validated at creation** — the predicate was never consulted, so every publish matching the `FROM` topic filter fired the rule's actions. It is now evaluated per publish: comparisons, `AND`/`OR`/`NOT`, parentheses, arithmetic, `IN`, `LIKE`, `BETWEEN`, `IS NULL`, `regexp_matches()`, under AWS's type conversions; a missing attribute never matches. Malformed SQL is rejected with `SqlParseException`. Contributed by @iot-rocket.
