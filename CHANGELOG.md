...
- **IoT Core — `sqs` topic-rule action** — a rule carrying `{"sqs": {"queueUrl", "roleArn", "useBase64"}}` was stored and then dispatched nothing, so the most common non-Lambda fan-out could not be rehearsed. The payload now goes through SQS's own `SendMessage`, so the queue's `DelaySeconds` and size limit apply; `useBase64` Base64-encodes the body as on AWS; an unresolvable queue fails that action alone and reaches the rule's `errorAction`; FIFO destinations are refused. Contributed by @iot-rocket.
