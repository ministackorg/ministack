...
- **IoT Core — fleet indexing** — `UpdateIndexingConfiguration` / `GetIndexingConfiguration` / `DescribeIndex` / `ListIndices` gate the `AWS_Things` index (OFF by default, as on AWS); `SearchIndex` then queries the live registry and shadows — `AND`-separated terms over `thingName`, `thingTypeName`, `thingGroupNames`, `attributes.*` and `shadow.desired|reported.*`, `*` / `?` wildcards, `maxResults` / `nextToken` paging. Searching with indexing off is a `ResourceNotFoundException`. Contributed by @iot-rocket.
