...
### Added
- **EventBridge — dead-letter queues now work for every target type, not just API destinations** — a target's `DeadLetterConfig` was only honored on the API-destination dispatch path; Lambda, SQS, SNS, Step Functions, CloudWatch Logs, and event-bus targets logged failures and dropped the event. The DLQ context is now resolved once per invocation and threaded through every dispatcher: a target resource that doesn't exist dead-letters as `NO_RESOURCE` **without retry attempts and without `EXHAUSTED_RETRY_CONDITION`** (per the eb-rule-dlq user guide, missing-resource errors skip the retry pipeline entirely), and an unexpected dispatch error dead-letters as `INTERNAL_ERROR`. The body remains the original pre-input-selection envelope with `RULE_ARN`/`TARGET_ARN`/`ERROR_CODE`/`ERROR_MESSAGE`/`RETRY_ATTEMPTS` attributes. `PutTargets` now also rejects a `DeadLetterConfig.Arn` that is not an SQS queue ARN with a `ValidationException`, as on AWS. Cross-region target failures still drop without dead-lettering (the live-verified FailedInvocations policy).
- **EventBridge — replay parity: `Destination.FilterArns`, `replay-name`, no re-archiving, `EventLastReplayedTime`** — `StartReplay` previously replayed to every rule on the bus and replayed events were indistinguishable from originals. Now: `Destination.FilterArns` limits which rules process replayed events; every replayed event carries the top-level `replay-name` field real EventBridge stamps (visible to target envelopes, `InputPath`, `InputTransformer` paths, and event patterns — the documented `"replay-name": [{"exists": false}]` anti-replay guard now works); replayed events are **never sent back to an archive** (per the CreateArchive contract, preventing archive self-amplification); events replay in event-time order; the replay record tracks `EventLastReplayedTime`; and `StartReplay` returns `ReplayStartTime`. `StartReplay` also validates the required `EventStartTime`/`EventEndTime` members, rejects `EventEndTime <= EventStartTime`, and caps replay names at 64 chars.
- **EventBridge — `CancelReplay` returns `IllegalStatusException` outside Running/Starting** — cancelling a `COMPLETED` replay drew a generic `ValidationException` and cancelling a `CANCELLED` one succeeded silently; the live API rejects both with `IllegalStatusException` ("a replay can be canceled only when the state is Running or Starting").
- **EventBridge — `CreateArchive` validates its inputs like the live API** — the source event bus must exist (`ResourceNotFoundException`), the event pattern must be valid JSON (`InvalidEventPatternException` — previously only `UpdateArchive` checked), and `RetentionDays` must be ≥ 0. Archives also account `SizeBytes` as events are captured instead of pinning it to 0.
- **EventBridge — API destination targets are now invoked over HTTP on `PutEvents`** — API destinations and connections were control-plane stubs: a matching rule with an api-destination target logged "unsupported event target ARN" and dropped the event, so webhook-style pipelines (EventBridge → HTTPS endpoint) could not be tested locally. Matching events now POST (or the destination's configured method) to the `InvocationEndpoint` with the input-selected payload (`Input` / `InputPath` / `InputTransformer` apply as for other targets). Connection authorization is honored: `BASIC` populates `Authorization: Basic …`, `API_KEY` sends the configured header, and `OAUTH_CLIENT_CREDENTIALS` authenticates the token request with **HTTP Basic auth** (`ClientID:ClientSecret`) — verified live against the EventBridge API; `grant_type`/`audience`/`scope` reach the token request only through `OAuthHttpParameters`, never implicitly — caches the token per connection, refreshes proactively when it expires within 60 seconds, and refreshes + retries once on a `401`/`407` response, matching documented AWS behavior. Connection `InvocationHttpParameters` and target `HttpParameters` are merged with connection values taking precedence (per the `HttpParameters` API reference), `PathParameterValues` populate `*` path wildcards, and body parameters fold into JSON-object bodies. Requests carry the AWS default headers (`User-Agent: Amazon/EventBridge/ApiDestinations` and `Range` non-overridable, `Content-Type` defaulting to `application/json; charset=utf-8`), strip the headers real EventBridge removes, and time out after 5 seconds (the documented maximum client execution timeout). Delivery runs on a background thread mirroring the SNS HTTP(S) path. Not modeled, mirroring the cross-region FailedInvocations policy: the 24h/185-attempt retry pipeline, `Retry-After`, and `InvocationRateLimitPerSecond` — retries collapse to a single attempt.
- **EventBridge — failed API destination deliveries dead-letter to the target's `DeadLetterConfig` queue** — a target's `DeadLetterConfig` SQS queue now receives failed deliveries with the message attributes real EventBridge stamps (`RULE_ARN`, `TARGET_ARN`, `ERROR_CODE`, `ERROR_MESSAGE`, `RETRY_ATTEMPTS`, and `EXHAUSTED_RETRY_CONDITION` when the retry policy would have run out). The body is the **original event envelope**, never the transformed target input, so DLQ redrive tooling sees what AWS would give it. Retryable statuses (`401`/`407`/`409`/`429`/`5xx`) dead-letter with `ERROR_CODE=ERROR_FROM_TARGET` and `EXHAUSTED_RETRY_CONDITION=MaximumRetryAttempts` (the retry pipeline itself stays unmodeled — delivery collapses to one attempt); non-retryable statuses dead-letter without retry, as on AWS; network failures map to `CONNECTION_FAILURE`/`TIMEOUT`; a non-ACTIVE destination or missing connection dead-letters as `NO_RESOURCE`. FIFO queues are rejected as DLQs, matching the AWS restriction.
- **EventBridge — CloudWatch Logs targets are dispatched on `PutEvents`** — a rule target pointing at a `logs` log-group ARN (with or without the `:*` suffix) now delivers each matched event as a new opaque-named log stream containing the (input-selected) event JSON, mirroring real EventBridge's per-delivery streams. Previously logs targets validated at `PutTargets` but were silently dropped at dispatch.
- **EventBridge — connections store their auth parameters in Secrets Manager and `DescribeConnection` is sanitized** — `CreateConnection` now mints a real Secrets Manager secret named `events!connection/<name>/<uuid>` holding the connection's auth parameters (readable locally via `GetSecretValue`, exactly like the service-owned secret on AWS) and surfaces its ARN as `SecretArn`; `UpdateConnection` refreshes the same secret and `DeleteConnection` deletes it. `DescribeConnection` now returns the `ConnectionAuthResponseParameters` shape — `Username`/`ApiKeyName`/`ClientID` survive, secret values never do, and http-parameter values flagged `IsValueSecret` are stripped — instead of echoing the raw stored record with secrets. `KmsKeyIdentifier` is accepted and echoed as `KmsKeyArn`.
- **EventBridge — archive `KmsKeyIdentifier` round-trip and explicit `DescribeArchive` shape** — `CreateArchive`/`UpdateArchive` accept `KmsKeyIdentifier` and `DescribeArchive` echoes it (the community EventBridge module ≥ v4.1 sets it); `DescribeArchive` now returns the API's response fields instead of the raw store record (which carries the captured events for replay).
- **EventBridge — server-side name caps** — `CreateArchive` rejects names over 48 chars and `CreateConnection`/`CreateApiDestination` reject names over 64 with AWS-shaped `ValidationException`s. Non-validating SDKs (the Terraform AWS provider among them) rely on the service for these, so plan-time guards built against the real limits now behave identically here.
- **EventBridge — object variables outside JSON value positions fail the invocation as `INVALID_JSON`** — AWS constrains input-transformer variables that resolve to a JSON object or array to JSON *value* positions (`{"detail": <detail>}`); a bare or quoted placement (`<detail>` as the whole template) is not a documented form and the live service fails the delivery with DLQ `ERROR_CODE=INVALID_JSON` / `Invalid input for target.` **before any HTTP request is made**. The emulator previously rendered such templates into perfectly valid JSON and delivered them, hiding the production failure. API-destination dispatch now detects the misplacement, skips the HTTP call, and dead-letters as `INVALID_JSON` when the target has a DLQ — the wrapped value-position form still delivers.
- **EventBridge — `PutTargets` validates static `Input` is JSON** — a target `Input` that is not valid JSON text is now rejected with a `ValidationException` at `PutTargets`, as on AWS, instead of being stored and delivered verbatim.
...
- **CloudWatch Logs — resource policies** — `PutResourcePolicy`, `DescribeResourcePolicies`, and `DeleteResourcePolicy` are now served (used by Terraform's `aws_cloudwatch_log_resource_policy`, e.g. to authorize EventBridge log delivery). The AWS limit of 10 resource policies per account per region is enforced — the 11th `PutResourcePolicy` fails with `LimitExceededException` while in-place updates still succeed — because the limit is not adjustable on AWS and fleet designs need to hit the same wall locally.
