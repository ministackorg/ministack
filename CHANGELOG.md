...
### Added
- **CloudWatch Logs — `GetLogRecord`, Insights `@ptr` rows, and `StartLiveTail`** — `StartQuery` / `GetQueryResults` previously stubbed empty results, so there was no way to obtain an Insights `@ptr` or round-trip `GetLogRecord`. `PutLogEvents` now assigns an opaque pointer per event; Insights queries return matching rows in the AWS field/value shape (including `@ptr`, `@timestamp`, `@message`, `@logStream`, `@log`); and `GetLogRecord` resolves those pointers to the full transformed field map (`unmask` accepted, masking not implemented). `StartLiveTail` is also implemented as a wire-valid `application/vnd.amazon.eventstream` response (`initial-response`, then `sessionStart` plus `sessionUpdate` frames over already-ingested matching events, capped/sampled at 500) so Live Tail clients can be integration-tested without AWS; long-lived 3-hour push sessions remain out of scope. `DescribeLogGroups` now returns both `arn` (with trailing `:*`) and `logGroupArn` (StartLiveTail-safe, no star). Full CWLI grammar remains out of scope — simple `fields` / `limit` parsing only. Contributed by @ovsteenb.
