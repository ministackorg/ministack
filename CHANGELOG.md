...
### Added
- **KMS — `GenerateRandom`** — the one keyless KMS operation answered `InvalidAction`, which surfaces as an unhandled 500 in any caller that mints random bytes through KMS (ours generates TOTP scratch codes with it). The operation now returns the requested 1-1024 bytes from `os.urandom`, refuses an omitted or out-of-range `NumberOfBytes` with the service's `ValidationException` wording, and answers `CustomKeyStoreNotFoundException` for a `CustomKeyStoreId` — MiniStack has no custom key stores. The Nitro-enclave `Recipient` parameter is ignored. Contributed by @iot-rocket.
