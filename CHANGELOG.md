...
- **Lambda — a Docker init failure or timeout is reported, not retried forever** — RIE answers an unimportable handler with a 502, and one that blows its `Timeout` with a read timeout; the invoke loop read both as "container not up yet" and re-POSTed ten times a second, re-running the handler. Both are final after one run now: 200 with `X-Amz-Function-Error: Unhandled` and the error payload, and a timed-out container is recycled, not pooled. Cold-start retries unchanged. Contributed by @iot-rocket.
