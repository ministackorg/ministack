...
- **IoT Wireless — `GetPositionEstimate`** — the service was entirely absent, so a handler that resolves a device's position from its IP (`Ip={"IpAddress": ...}`) could not run at all. `POST /position-estimate` now answers the way the API is actually shaped: the output structure declares a `payload` blob, so the HTTP body IS the raw GeoJSON `Point` and boto3 hands the caller a `StreamingBody` — a JSON envelope here would break every SDK client. The estimate is synthetic and deterministic (the canonical `Ip.IpAddress` hashes onto lon [-180, 180) / lat [-60, 60)), so the same IP always answers byte-identical GeoJSON and a consumer test can assert on it — the live service's payload properties (`country`, `horizontalAccuracy`, `horizontalConfidenceLevel`, `timestamp`, plus city/state/postal code on IP lookups) are trimmed to the two accuracy fields for that determinism; `WiFiAccessPoints`, `CellTowers`, `Gnss` and `AdvancedConfiguration` are accepted and ignored — resolving is IP-only. Refusals carry the live service's verbatim wordings: no resolver hint and an address `ipaddress.ip_address()` refuses each answer the measured `ValidationException` message, and a valid address the resolver cannot place answers `ResourceNotFoundException` `Cannot find position for the IP address <ip>` — the real service refuses when its geo database has no entry; MiniStack approximates that refusal class deterministically by refusing every address that is not globally routable (private, loopback, TEST-NET, ...). Contributed by @iot-rocket.
