...
- **IoT Core — CA-certificate registry and registration events** — `RegisterCACertificate` with its describe/update/list/delete siblings and `GetRegistrationCode` / `DeleteRegistrationCode`. `RegisterCertificate` checks `caCertificatePem` — the CA must be registered and have signed the leaf, else `CertificateValidationException` — links `caCertificateId`, and under an ACTIVE auto-registering CA publishes the AWS event to `$aws/events/certificates/registered/{caId}`. Contributed by @iot-rocket.
...
- **IoT Core — deleting an ACTIVE certificate answers `406`** — `DeleteCertificate` returned `CertificateStateException` as `409`, so a client keying off the HTTP status saw a conflict where AWS reports not-acceptable, and it disagreed with the CA-certificate delete on the very same exception. Both are `406` now. Contributed by @iot-rocket.
