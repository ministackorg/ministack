...
### Fixed
- **Lambda — the layers AWS publishes from its own accounts attach** — the CloudWatch Lambda Insights, Parameters and Secrets, AppConfig, ADOT and SDK for pandas layers are referenced by the publisher's ARN, and AWS grants `lambda:GetLayerVersion` on them to everyone; after the grant evaluation landed, the emulator still refused them because no grant is stored for an account it never saw. A layer version from a documented publisher account in the request's region is now answered with that public grant, as a record without content, and an attachment registers it under the publisher account: `GetLayerVersionByArn` answers it, the function runs without the extension, and the attachment reports a `CodeSize` of 0. Any other foreign account, the publisher account in another region, and a name the publisher account registered locally keep following the stored grants. Contributed by @iot-rocket.
