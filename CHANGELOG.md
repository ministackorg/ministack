...
- **IoT — `UNSUBSCRIBE` removes only the topic filters it names** — the MQTT-over-WebSocket bridge never read the packet's payload and dropped every subscription the session held, so a client unsubscribing from one topic silently stopped receiving all the others. Named filters now match the session's subscriptions character-by-character (a wildcard goes by its own text), the rest keep delivering, and an unknown filter is a no-op that is still acknowledged. Contributed by @iot-rocket.
