...
- **Step Functions — a failed `lambda:invoke` task sets `Cause` to a JSON-encoded error payload** — the `Cause` of a Lambda task failure was the bare `errorMessage` string instead of AWS's JSON object (`{"errorType": ..., "errorMessage": ..., "trace": [...]}`). Catch handlers and downstream tasks that `json.loads(Cause)` to read `errorType`/`errorMessage` failed to parse it; `Cause` now matches the AWS wire form.
