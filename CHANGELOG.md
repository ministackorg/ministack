...
- **CloudFormation — `AWS::IoT::CACertificate` provisions onto the CA registry** — the registry API has carried `RegisterCACertificate` since 1.4.19, but a template declaring `AWS::IoT::CACertificate` still rolled the stack back with `Unsupported resource type`, so a JITR fleet's one foundational resource had to be registered out of band. The provisioner now drives the real API: create registers the PEM (idempotently, adopting an existing registration under the same PEM-derived id), update applies `Status` / `AutoRegistrationStatus` / `CertificateMode` in place, and delete deactivates the CA first, because an ACTIVE CA refuses `DeleteCACertificate`. A changed `CACertificatePem` fails the update loudly — the physical id is derived from the certificate content, and silently replacing a CA would orphan every device certificate registered under the old one. `RegisterCACertificate` also stores `certificateMode` now (`DEFAULT` when omitted, `InvalidRequestException` for an unknown value) and `DescribeCACertificate` reports it. Contributed by @iot-rocket.
