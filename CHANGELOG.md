...
- **Persistence — four services keep their state across a restart** — the save map was maintained by hand, and `cloudcontrol`, `config`, `cur` and `organizations` were never added to it, so their state was dropped on every warm boot. It is now derived from `SERVICE_REGISTRY`, and a test refuses any service that would be saved without a way to restore it. Contributed by @jgrumboe.
