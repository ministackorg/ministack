...
### Added
- **IoT Jobs — delivered over MQTT** — the jobs data plane existed only as the `iot-jobs-data` HTTP API; nothing was ever published on `$aws/things/<t>/jobs/#`, so a real device firmware speaking the AWS IoT Jobs MQTT contract (subscribe `notify-next`, publish `start-next`/`update`) hung forever and test harnesses had to run a hand-rolled responder next to the emulator. The reserved request topics (`get`, `start-next`, `<jobId>/get` — including the documented `$next` sentinel — and `<jobId>/update`) now answer on their `accepted`/`rejected` topics from the same store the HTTP plane uses, with `jobDocument` served as a JSON object (over HTTP it stays a string, as on AWS) and rejections carrying the string `ErrorCode` values (`InvalidRequest`, `InvalidJson`, `ResourceNotFound`, `VersionMismatch`, `InvalidStateTransition`, `TerminalStateReached`) plus the execution's current state on the version and state-transition conflicts. `notify` fires when an execution joins or leaves the pending set (job create, cancel, and delete, and terminal device reports) and `notify-next` only when the job at the front of the queue changes — starting the first job fires neither, matching a capture against live AWS. Mirrors the shadow-over-MQTT bridge, including its recursion guard. Known boundaries: the request members `stepTimeoutInMinutes` and `executionNumber` are accepted but ignored (there are no execution timeouts, and an execution is never re-queued); a CONTINUOUS job's execution materialized lazily for a thing that joined the target group later does not notify; and DeleteThing's execution sweep does not notify the deleted thing (the sweep is itself a divergence — AWS keeps a deleted thing's executions, so it publishes nothing there either). Contributed by @iot-rocket.
...
### Fixed
- **IoT Jobs — device-plane timestamps were milliseconds, 1000x off** — `iot-jobs-data` responses (`GetPendingJobExecutions`, `StartNextPendingJobExecution`, `DescribeJobExecution`) served `queuedAt`/`startedAt`/`lastUpdatedAt` as raw epoch milliseconds, but the API reference words every one of these `long` fields as "The time, in seconds since the epoch" (a live capture agrees: 10-digit values), so a device diffing a job stamp against its own clock saw dates around the year 58580. Every device-plane stamp — the HTTP API and the new MQTT bridge alike — is now whole epoch seconds; records keep millisecond resolution internally so same-second executions still hand out in queue order. Contributed by @iot-rocket.
