...
- **S3 — a presigned upload is no longer rejected for the checksum in its query string** — since v3.729.0 the JavaScript SDK signs `x-amz-checksum-crc32` into every presigned `PutObject` by default, computed over the *empty* body, because whoever holds the URL chooses the body later. MiniStack hoisted that parameter into the request headers alongside the user metadata a presigned URL legitimately carries there, and the header path then verified it against the real body, so a stock `getSignedUrl` upload failed with `400 BadDigest` where real S3 answers `200` and stores the object. The `x-amz-checksum-*` value parameters are now excluded from hoisting: they still take part in signature verification, so rewriting one in the URL is still `403`, but they are no longer read as a supplied integrity value. A checksum sent as a real request header is verified exactly as before. `x-amz-checksum-algorithm` and `x-amz-sdk-checksum-algorithm` keep hoisting — they name an algorithm for the server to compute rather than carrying a value, so they cannot disagree with a body, and a presigned `CreateMultipartUpload` uses either to choose the algorithm its parts are digested with. Contributed by @bognari.
...
- **Transcribe — `StartTranscriptionJob` returns a job already `IN_PROGRESS`** — the start response reported `QUEUED` with no `StartTime`, and the job reached `IN_PROGRESS` only from the background worker, after the response had been sent. A caller that reads the status off the start response and then waits for the terminal `Transcribe Job State Change` event, rather than polling, therefore never saw an in-progress state at all: the job went straight from queued to complete. Jobs now start `IN_PROGRESS` with `StartTime` set, matching the response the AWS CLI walkthrough documents, and `TRANSCRIBE_JOB_RUN_SECONDS` covers the whole run rather than half of it. `QUEUED`, which AWS reaches only for a request that opted into job queueing through `JobExecutionSettings.AllowDeferredExecution` while the account is at its concurrent job limit, is not modelled, so a job never queues. `TRANSCRIBE_JOB_RUN_SECONDS` is settable at runtime through `/_ministack/config`. Contributed by @ppettitau.

- **Lambda — a Node custom resource signals its stack in the docker executor** — the response submitters the CDK bundles into its custom-resource handlers (`nodejs-entrypoint`, the provider framework, `AwsCustomResource`) build the `ResponseURL` PUT from the URL's hostname and path only and hand it to `https.request`, so under `LAMBDA_EXECUTOR=docker` the callback went out over TLS to port 443, nothing answered, and an `AwsCustomResource`-backed resource ran its API call and then hung its stack until `ServiceTimeout` (the local executor already downgraded https for the gateway). The shim the docker executor injects now turns https to the gateway hosts (the endpoint host, `host.docker.internal`, `localhost` and `MINISTACK_HOST`) on the https default port or the gateway port into plain http on the gateway port; other hosts, and those hosts on other ports, keep TLS. The local executor's own list gains `MINISTACK_HOST` as well. Contributed by @iot-rocket.

- **Lambda — a durable invocation runs on the warm pool** — durable Python and Node.js invocations were sent to the one-shot executor, because the three `AWS_LAMBDA_DURABLE_*` variables change on every call while a pooled worker's environment is fixed when it spawns. Each of them therefore paid a fresh interpreter start and the botocore import inside the function's `Timeout`, whose default is 3 seconds, so on a loaded machine every durable invocation ended as a timeout. The values now travel in the event, the channel the X-Ray trace header and the invoke-depth counter already use, and both worker bootstraps move them into the environment before the handler runs and drop them again when the next invocation is not durable. Contributed by @iot-rocket.

- **Lambda — two durable executions of one function no longer share a callback** — the `CallbackId` handed to the SDK was the operation id, which the durable SDK derives from the workflow position, so two concurrent executions of the same function registered the same id: the second overwrote the first, an answer sent for one completed the other, and the abandoned execution's own callback then answered `CallbackTimeoutException`. The id is now unique per execution and stable across replays and restarts. Reported by @Nhollas.
...
- **CloudFormation — `Fn::GetAtt` on a user pool client's `ClientSecret` resolves** — the provisioner returned `ClientId` alone, so a template reading `ClientSecret` or `Name` failed the resource with `Requested attribute ClientSecret does not exist in schema` and rolled the whole stack back, on the create and on the update that follows it — the shape Serverless Framework deploys. All three attributes the type declares are returned now, on both paths; a client created without `GenerateSecret` has no secret and the attribute reads empty. Reported by @JoshuaSmeda.
...
- **CloudFormation — `AWS::Lambda::Alias` updates in place, and `ProvisionedConcurrencyConfig` reaches the service** — the type had no update handler and its create ignored `ProvisionedConcurrencyConfig` and stored `RoutingConfig` in the template's list shape (measured: `GetProvisionedConcurrencyConfig` on the alias answered `ProvisionedConcurrencyConfigNotFoundException` right after the deploy, and boto3's `GetAlias` on an alias with a `RoutingConfig` failed on the list where it expects a map). `FunctionVersion`, `Description`, `RoutingConfig` and `ProvisionedConcurrencyConfig` are No interruption on the resource reference and now go through `UpdateAlias` and `PutProvisionedConcurrencyConfig` on the existing alias, a dropped `Description` empties, a dropped `RoutingConfig` or `ProvisionedConcurrencyConfig` is removed, and `Name` or `FunctionName` replaces the alias. The routing weights are stored as the `{version: weight}` map `GetAlias` returns, on create as well. Contributed by @iot-rocket.

- **CloudFormation — `AWS::Logs::SubscriptionFilter` updates in place, and a group change moves it** — the type had no update handler; a changed `LogGroupName` fell through to create, which wrote the filter on the new group under its unchanged name, so the engine saw no replacement and the old group kept its copy (measured: two groups carried the filter after the update). `FilterPattern`, `DestinationArn`, `RoleArn` and `Distribution` are No interruption on the resource reference and now go through `PutSubscriptionFilter` on the existing filter, a dropped property reverts to what the create stores without it, and a changed `FilterName` or `LogGroupName` creates the new filter before the old one is removed. Contributed by @iot-rocket.

- **CloudFormation — `AWS::IAM::InstanceProfile` updates in place, and a named profile refuses a `Path` change** — the type had no update handler, so a `Roles` change fell through to create, which rebuilt the record with no tags and stored the roles as whole role records, on which `GetInstanceProfile` failed (measured: HTTP 500 on any CloudFormation-provisioned profile that has a role); a `Path` change on a custom-named profile was silently executed. `Roles` is No interruption on the resource reference and is now applied to the existing record, which keeps its ARN, id and API-set tags; `InstanceProfileName` and `Path` are Replacement, so a custom-named profile whose `Path` changes fails the update with CloudFormation's "cannot update a stack when a custom-named resource requires replacing" and a generated-name profile is re-created under the new path. The create stores role names, so the profile reads back. Contributed by @iot-rocket.
...
- **CloudFormation — `AWS::CloudFront::Distribution` updates in place, and a provisioned distribution is readable** — a stack update that changed a distribution fell through to the create handler: a new `Id` and `DomainName` on every change, the old distribution deleted by the engine, while `DistributionConfig` and `Tags`, the type's two properties, are both *No interruption* on the resource reference. And the create stored an empty configuration, so `GetDistribution` and `GetDistributionConfig` answered a 500 on every CloudFormation-provisioned distribution and `ListDistributions` showed empty blocks. The create now renders the template's `DistributionConfig` into what `CreateDistribution` stores, in the API's shape (a plain list becomes a `Quantity`/`Items` block, `CachedMethods` nests under `AllowedMethods`, `IPV6Enabled` is `IsIPV6Enabled`, `OriginCustomHeaders` is `CustomHeaders`, an empty block carries no `Items`, the legacy `CNAMEs`, `CustomOrigin` and `S3Origin` are dropped), with a `CallerReference` of its own and the resource `Tags`; the update keeps `Id`, `ARN`, `DomainName` and the invalidation history, stores the new configuration, rolls the `ETag`, refreshes `LastModifiedTime` and reconciles the tags, as `UpdateDistribution` does; deleting the distribution removes its tags with it. Now that the type has a tag store, it joins the table that merges the stack's own tags and the three `aws:cloudformation:` tags into a resource, as every other tag-storing type already did. Contributed by @iot-rocket.

- **API Gateway v2 — `UpdateApi` keeps the description and the API key selection expression** — the call applied five members and silently discarded `Description` and `ApiKeySelectionExpression`, so an update through the API lost what the create had stored. Both are applied now, and a `CorsConfiguration` on the call replaces the stored one; removing a configuration is `DeleteCorsConfiguration`, the operation the API defines for it, which `DELETE /v2/apis/{apiId}/cors` now serves. Contributed by @iot-rocket.

- **CloudFormation — a nested stack's IAM resources need the parent's capabilities** — the check under `AUTH=true` read the parent template only, so a parent deployed without `--capabilities` provisioned a child full of IAM roles through `AWS::CloudFormation::Stack`. The child template is now checked against the set the parent acknowledged when it is read, and a child's child reads the same set, as AWS asks for the capabilities on the parent for every level of nesting; a missing one fails the nested-stack resource with the message of `InsufficientCapabilitiesException` (`Requires capabilities : [CAPABILITY_IAM]`) and the parent rolls back unless rollback is disabled. Under `AUTH=true` the child's record carries the parent's set, so `DescribeStacks` reports it there too; without `AUTH` nothing is recorded and nothing changes. Contributed by @iot-rocket.

- **CloudFormation — the SAM transform reads the transforms the response reports** — `ValidateTemplate` normalises the `Transform` section in every form it takes, but the deploy path had its own normalisation that left the macro object (`Transform: {Name: AWS::Serverless-2016-10-31}`) in the list, so a template written that way was reported as declaring the SAM transform and then deployed untransformed: its `AWS::Serverless::Function` resources stayed as they were and the stack failed on an unsupported type. Both read one function now, so the response and the deploy agree about what a template declares. Contributed by @iot-rocket.
...
- **CloudWatch Logs — a log group reports its class and its KMS key** — `DescribeLogGroups` omitted `logGroupClass` and `kmsKeyId` entirely, and `CreateLogGroup` discarded both, so a consumer that reads the class back saw an empty value and planned a replacement of a log group that had not changed (Terraform's `aws_cloudwatch_log_group` with `log_group_class = "STANDARD"`). Both are stored on creation and reported on every group, with the API's default of `STANDARD` when the request names no class and an unknown class refused, and `AWS::Logs::LogGroup` carries `LogGroupClass` and `KmsKeyId` through the stack. Reported by @edersonbrilhante.

- **SSM — a `SecureString` parameter reports its `KeyId`** — the key was stored and never served: `DescribeParameters` and `GetParameterHistory` left the member out, so a caller read an empty key and updated the parameter on every run (Terraform's `aws_ssm_parameter` expects `alias/aws/ssm` for the account's default key). Both now report it for the parameters it applies to, `SecureString` only, and a request that sends an empty `KeyId` gets the account default rather than an empty string. `GetParameter` is unchanged, since its response shape has no such member. Reported by @edersonbrilhante.

- **EC2 — a security group's IPv6 and prefix-list rules survive the read** — `DescribeSecurityGroups` rendered `ipv6Ranges` and `prefixListIds` as empty elements and dropped every range description, so a rule authorized with `Ipv6Ranges` or a prefix list was invisible on the next read and Terraform planned the same egress change on every run. All three families are reported now, each with its description. Reported by @edersonbrilhante.
...
- **CloudFormation — the five CloudFront policy, OAC and function types update in place** — `AWS::CloudFront::CachePolicy`, `OriginRequestPolicy`, `ResponseHeadersPolicy`, `OriginAccessControl` and `Function` had create handlers only, so a stack update fell through to the create, which found the record it had made before and returned it untouched: the stack reported `UPDATE_COMPLETE` while the service kept serving the create-time configuration. Each type now updates its record, keeping the id a distribution `Ref`s, and a rename onto a name another resource already holds is refused rather than writing over it. Contributed by @iot-rocket.

- **Lambda — an SDK call from a function runs under its execution role** — under `AUTH=true` every runtime received account-root credentials, so a call from a handler bypassed the role the function declares and a policy that should have denied it did nothing. Each invocation now receives temporary credentials for its `Role` across the warm workers, the Docker and provided runtimes and the one-shot executor, and the IAM layer resolves the assumed role's account, a DynamoDB table's index ARNs and its projection condition keys, and the bus ARNs of an EventBridge `PutEvents`. Contributed by @adamkeener.

- **ECS — `RunTask` returns before the image is pulled** — the call blocked while Docker pulled the image and started the container, so a caller waited out the whole pull for a task the API reports as starting. A task is registered and returned immediately, the container starts in the background, and the task walks `PENDING` → `ACTIVATING` (where the pull happens, with `pullStartedAt` stamped) → `RUNNING` → `STOPPED`, with a failed start reported as `TaskFailedToStart`. `StopTask` and a reset can no longer race the starter, and `pendingCount` and `runningCount` keep counting only `PENDING` and `RUNNING` tasks, as the API documents. Contributed by @po-luka-miletic.

- **S3 — a presigned URL is verified against the credentials that signed it** — the signature was always recomputed with the server's own secret, so a URL signed with an IAM user's or an STS session's key verified against the wrong secret, and a key that had been deactivated, a session that had expired or a session token that was never issued were all accepted. The signing key is now resolved to its owner — root, IAM user or STS session — and the URL is verified with the secret that key carries, with the exact session token required for temporary credentials; an unknown or inactive key is `InvalidAccessKeyId` (403), an expired or malformed token `ExpiredToken` or `InvalidToken` (400), the statuses S3's error table gives. Under `AUTH=true` an HTTP request also resolves its IAM key to the owning account, `GetSessionToken` refuses temporary credentials, and an assumed role's policies are read from the session's account. Contributed by @Areson.
