...
- **CloudFormation — `AWS::ApiGatewayV2::Authorizer` survives a property update** — the type had no update handler, so changing e.g. `JwtConfiguration.Audience` (trusting an additional app client) fell back to create — whose `authorizerId` is a random id on every call, with no stable name to derive from — minting a second, orphaned authorizer while every `Route`'s own (unchanged) `AuthorizerId` kept pointing at the original, now-stale one. A real update handler now mutates the existing authorizer record in place, matching API Gateway's own `UpdateAuthorizer`. Contributed by @ryan-bennett.
...
- **S3 — `GetBucketLocation` returns the bucket's stored region** — two defects hid the region: a bucket created without a `CreateBucketConfiguration` stored no region at all (even when the request was signed for one), and the response suppressed the region whenever it equalled `MINISTACK_REGION`, so with the emulator homed in, say, `eu-central-1` a bucket in that region answered an empty `LocationConstraint` — which S3 clients read as us-east-1 and then mis-sign follow-up requests with. A bucket created with no explicit constraint now lands in the request's SigV4 signing region, as on AWS, and `GetBucketLocation` returns the empty `LocationConstraint` only for buckets literally in `us-east-1`, echoing every other region verbatim.
- **S3 — versioned objects retain their custom metadata** — in a versioning-enabled bucket the per-version record stored only the wire basics (etag/size/storage class), dropping the object's `x-amz-meta-*` user metadata, preserved headers, and content-encoding; on top of that a `GetObject?versionId=` hand-built a five-header response that bypassed the only `x-amz-meta-*` emitter. A `PutObject` with `Metadata` followed by a `GetObject`/`HeadObject` of that version therefore came back with the metadata missing. Every version now stores its own metadata (across put, POST-form, copy, and multipart-complete), and versioned reads route through the shared header builder, so each version returns the metadata it was written with — matching S3, where metadata is per-version. Reported by @Kaphaalor.
- **API Gateway (REST) — custom Lambda authorizers are invoked and enforced on the request path** — a method with `authorizationType: CUSTOM` stored its authorizer but never called it: requests reached the integration unauthenticated and `requestContext.authorizer` was never populated. `TOKEN` and `REQUEST` authorizers are now invoked on the data path with the AWS event shape (`type`/`authorizationToken`/`methodArn`, or the REQUEST parameter shape); a missing identity source returns `401` without invoking, the returned IAM policy is evaluated against the method ARN (explicit `Deny` and no-matching-`Allow` both `403`, an authorizer raising `Unauthorized` `401`), `authorizerResultTtlInSeconds` caches the decision, and on allow the returned `context` (values stringified) plus `principalId` are injected into `requestContext.authorizer`. `AWS_IAM` methods reject a request with no `Authorization` header (`403`); the SigV4 signature itself is not verified. Reported by @iot-rocket.
- **Route 53 — a change is born `PENDING` and flips to `INSYNC`** — every `ChangeResourceRecordSets` / `CreateHostedZone` change was created `INSYNC`, so `GetChange` never returned `PENDING` and any waiter or poll loop (e.g. the `ResourceRecordSetsChanged` waiter) completed on its first read without exercising its retry path. Changes are now submitted `PENDING` and flip to `INSYNC` on the first `GetChange` read thereafter, reproducing the observable `PENDING`→`INSYNC` transition without a background timer. Reported by @jayjanssen.
