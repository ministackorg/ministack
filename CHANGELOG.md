...
- **IAM — three calls are authorized against the action and resource AWS uses** — a Function URL invoke is checked against the function ARN with its alias qualifier and supplies `lambda:FunctionUrlAuthType`; `@connections` asks for `execute-api:ManageConnections`; `iot-jobs-data` operations move to `iotjobsdata:`, except `StartCommandExecution`. Contributed by @iot-rocket.
