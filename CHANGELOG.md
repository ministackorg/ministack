...
### Added
- **EKS — Pod Identity associations** — the five operations did not exist, so a controller that reads pod identity to find its role, the AWS Load Balancer Controller among them, could not run against MiniStack. `CreatePodIdentityAssociation`, `DescribePodIdentityAssociation`, `ListPodIdentityAssociations` (filtered by `namespace` and `serviceAccount`), `UpdatePodIdentityAssociation` and `DeletePodIdentityAssociation` serve the records, one per namespace and service account, with an `externalId` minted for a `targetRoleArn` chain. Reported by @josephaw1022.
- **EKS — `aws eks update-kubeconfig` works against MiniStack** — k3s could not authenticate the IAM exec token `aws eks get-token` produces, so the only way in was to copy the admin kubeconfig out of the container. MiniStack now answers a TokenReview webhook for the cluster: `AUTH=false` accepts local bearer tokens, `AUTH=true` verifies the presigned token's signature, cluster binding and lifetime and then requires a creator or Access Entry grant, including account-scoped STS sessions. The five general-purpose access policies (`AmazonEKSClusterAdminPolicy`, `AmazonEKSAdminPolicy`, `AmazonEKSEditPolicy`, `AmazonEKSViewPolicy`, `AmazonEKSAdminViewPolicy`) grant their published Kubernetes permissions, at cluster or namespace scope including wildcard patterns, and disassociating a policy or deleting its Access Entry revokes them. Contributed by @jgrumboe. Reported by @StraggleCraft.
...
- **ECS — a task walks the whole lifecycle** — a Docker-backed task went straight from `PENDING` to `RUNNING` to `STOPPED`, so a consumer waiting on any other state waited forever. It now reports the states AWS documents, in order: `PROVISIONING` at `RunTask` whatever the network mode, `PENDING` while it waits on the agent, `ACTIVATING` for the image pull, then `DEACTIVATING`, `STOPPING`, `DEPROVISIONING` and `STOPPED`. Every starting state counts against a service's desired capacity, while `pendingCount` and `pendingTasksCount` still count `PENDING` alone as the API model defines them. `executionStoppedAt` is now set. Reported by @iot-rocket.
- **Cognito — a hosted-UI request that invokes a Lambda trigger no longer deadlocks** — `/saml2/idpresponse`, `/oauth2/idpresponse` and `/oauth2/token` ran on the event loop, so a PreSignUp or PreTokenGeneration trigger calling back into MiniStack queued behind its own request, timed out and surfaced as a `400`. All three now run off the loop. Two races the change exposed close with it: a replayed authorization code gets `invalid_grant`, the error AWS returns for a code "consumed already or does not exist", instead of a `500`. Contributed by @kjdev.
- **SQS — every batch action validates the request before its entries** — the ten-entry limit reached `SendMessageBatch` only, and three more request-level errors the SQS model declares on all three batch actions were never raised. `DeleteMessageBatch` and `ChangeMessageVisibilityBatch` now refuse an empty `Entries` list, more than ten entries, repeated ids and a malformed id, with `EmptyBatchRequest`, `TooManyEntriesInBatchRequest`, `BatchEntryIdsNotDistinct` and `InvalidBatchEntryId`. These fail the whole request rather than landing in `Failed`, as on AWS. Contributed by @CaptainAni187. Reported by @jespinoza-shippo.
- **SES — one v2 implementation, returning the v1 `MessageId` shape** — `ses.py` carried an inline copy of the v2 endpoints that duplicated `ses_v2.py`, and the path router had already been sending `/v2/email/*` to `ses_v2`, so the copy was unreachable and the two had drifted. The duplicate is gone and every v2 request, by path or by `SESv2.*` target header, is served by one implementation. That surfaced a divergence the dead copy had hidden: `SendEmail` and `SendBulkEmail` answered an id prefixed `ministack-`, where the six v1 send paths return `<uuid>@email.amazonses.com`. Both now return the shared shape. Contributed by @jgrumboe.
- **EKS — a deleted cluster takes its access entries and pod identity associations with it** — both stores are keyed by cluster name and neither was swept on `DeleteCluster`, so they outlived the cluster and a new one created under the same name inherited the previous cluster's grants and role ARNs. They are now removed with the cluster, along with their tags.
- **Persistence — four services keep their state across a restart** — the save map was maintained by hand next to `SERVICE_REGISTRY`, and `cloudcontrol`, `config`, `cur` and `organizations` were never added to it, so everything they held was dropped on every warm boot. The map is now derived from the registry, which also covers the modules a service reaches through its own dispatcher, and a test refuses any service that would be saved without a way to restore it. Every service module now declares the same `get_state` / `load_persisted_state` / `reset` contract, which is what the derivation reads. Contributed by @jgrumboe.
- **IAM — a resource-scoped policy matches, instead of denying** — under `AUTH=true` the resource ARN resolved to `*` for whole families of request, so only a grant written `Resource: "*"` matched and everything narrower was denied. IoT jobs and provisioning templates now resolve to `job/<id>` and `provisioningtemplate/<name>`, both IoT data planes reach the same branch, an MQTT topic resolves to its full multi-level path, and an API Gateway invoke resolves to `<api-id>/<stage>/<METHOD>/<path>`. A query-protocol POST carries its whole form body into resolution, not just `Action`. Contributed by @iot-rocket.
- **CloudFormation — `AWS::IAM::Policy` is provisioned inline, not as a managed policy** — the document was stored as an account-global managed policy and attached, where CloudFormation embeds it on each Role, User and Group it names, as `PutRolePolicy` does. An inline policy is scoped to its entity, so the same `PolicyName` can exist on many at once; CDK derives that name from the construct path, so stacks sharing a path collapsed onto one record and each deploy took the grants from the ones before it. `ListRolePolicies`, `GetRolePolicy` and `GetAccountAuthorizationDetails` now report it and the IAM evaluator finds it. `AWS::IAM::ManagedPolicy` is unchanged. Contributed by @iot-rocket.
- **Lambda — SQS event source batches are dispatched concurrently** — one thread polled every mapping and waited for each invoke, so a handler that took 10 seconds held a message on an unrelated queue for about 8.6 seconds, and `ScalingConfig.MaximumConcurrency` was stored but ignored. Batches now run off the poll thread, up to `MaximumConcurrency` per mapping (default 5), lowered to the function's `ReservedConcurrentExecutions`. A FIFO event source is held to one batch at a time whatever that setting says, since a batch is built across message groups and Amazon SQS "ensures that messages in the same group are delivered to Lambda in order". Kinesis and DynamoDB Streams still process one batch at a time. Contributed by @ThailerL.
- **ECS — a task's `version` counts its state changes** — the counter was minted at `1` and never moved, so a consumer holding an older `DescribeTasks` copy could not tell it from the current one, which is the comparison the field exists for. It now moves on each change the record reports, measured against a real Fargate task: `1` at `PROVISIONING`, `2` at `PENDING`, `3` at `RUNNING`, `4` when `StopTask` set `desiredStatus`, `5` at `DEPROVISIONING`, `6` at `STOPPED`. Contributed by @iot-rocket.
...
- **ECS — the task metadata endpoint reports the status the task is in** — `${ECS_CONTAINER_METADATA_URI_V4}` served `DesiredStatus` and `KnownStatus` as the literal `RUNNING`, captured at registration, so a container reading its own metadata was told the task was running while `DescribeTasks` reported otherwise, for the length of the image pull. Both are now seeded from the task record and pushed at each transition. `KnownStatus` is the task's on the task payload and the container's own on a container payload, because AWS reports them separately. Contributed by @iot-rocket.
- **IAM — one `_account_from_arn`, with the contract the condition evaluator documents** — the module carried two top-level definitions under the same name, so the later silently won and the guards the documented one describes were not the ones in force when `aws:ResourceAccount` resolved a resource's account. They are folded into one: the field is returned only when it holds a twelve-digit account id, and only from a value that is an ARN. No request changes behaviour. Contributed by @iot-rocket.
