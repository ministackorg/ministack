...
### Fixed
- **S3 Tables / Glue — the Iceberg REST catalogs honour `upgrade-format-version`** — a commit carrying the Iceberg REST `UpgradeFormatVersionUpdate` action (what a Spark job writing with the `format-version` `3` table property sends for an existing table) was silently dropped, so the table's metadata kept reporting version 2 while the commit answered success; real S3 Tables supports Iceberg format version 3. Both catalogs now apply the action: the metadata moves to the requested version, re-asserting the current version is a no-op, and a downgrade or a version above 3 is refused with 400 — committing nothing, since a commit is atomic (the S3 Tables catalog now stages updates on a copy, the Glue catalog never advances its `metadata_location` on a refusal). Creating a table with an explicit `format-version` already worked; the default stays 2.
...
- **CloudFormation — `DescribeStackResource` and `DescribeStackResources` carry `ResourceStatusReason`** — a resource in `DELETE_FAILED` (or any other failed state) kept its reason in the stack events only; the two Describe responses now carry the `ResourceStatusReason` element the API reference defines, absent for a healthy resource; `DescribeStackResource` also names its timestamp `LastUpdatedTimestamp`, the field `StackResourceDetail` defines (it carried a `Timestamp` element that clients drop, so the detail had no timestamp at all). Contributed by @iot-rocket.
- **CloudFormation — a removed resource whose delete fails stays in the stack as `DELETE_FAILED`** — when a resource dropped from the template could not be deleted in the cleanup after an update, the stack ended `UPDATE_COMPLETE` (as on AWS) but the resource was dropped from the stack record while it kept existing in the service, and `DeleteStack` never tried again. It now stays listed with `DELETE_FAILED` and the reason (in a nested stack too), the next update with a changed template retries the delete, and `DeleteStack` also reaches the resources the template no longer declares. Contributed by @iot-rocket.
- **S3 — the Multi-Region Access Point alias starts with a letter and follows the documented pattern** — the alias was the first 13 hex characters of a uuid, so it could come out as digits only (`1240729989764.mrap`), which is not what S3 mints (`mfzwi23gnjvgw.mrap`) and made `test_cfn_s3_multi_region_access_point` fail whenever no letter was drawn (`str.islower()` is False without a cased character). The alias now follows the documented pattern `^[a-z][a-z0-9]*[.]mrap$`: a letter, then twelve lowercase letters or digits; the test checks that pattern. Contributed by @iot-rocket.
### Added
...
- **CloudFormation — a `WaitCondition` waits for its signals** — `AWS::CloudFormation::WaitCondition` completed at once and `AWS::CloudFormation::WaitConditionHandle` answered a URL nothing served, so a stack never waited for the configuration step it was written to wait for. The handle's `Ref` (and physical id) is now a URL the emulator serves (`PUT` the AWS signal JSON, `Status`, `UniqueId`, `Data`, `Reason`, to it with an empty `Content-Type`; `Id` is its token); the wait condition stays `CREATE_IN_PROGRESS` until `Count` SUCCESS signals with distinct `UniqueId`s arrived (a repeated id is a retransmission and is ignored), a FAILURE signal or the `Timeout` fails it and rolls the stack back, every signal is published to the stack events, and `Fn::GetAtt Wait.Data` is the `{UniqueId: Data}` map. A wait condition with a `CreationPolicy` (`ResourceSignal` with `Count` and an ISO 8601 `Timeout`, default `PT5M`) needs no handle and is signalled through `SignalResource`. Without a `CreationPolicy` a missing `Timeout`, a `Timeout` outside 1..43200 seconds, a `Count` below 1 or a handle of another stack fail the resource. A nested stack now deploys on a worker thread, so a wait condition or custom resource inside it no longer blocks the server. The custom-resource `ResponseURL` honours `EDGE_PORT` like the rest of the emulator. Contributed by @iot-rocket.
- **CloudFormation — `SignalResource`** — the action did not exist (`InvalidAction`). It now delivers a `SUCCESS` or `FAILURE` signal with a `UniqueId` to the wait condition that is waiting under the given logical id, addressed by stack name or id; a missing field, a stack that is not in progress, a resource that is not waiting, a `Status` outside the enum or a `UniqueId` longer than 64 characters are a `ValidationError`. Contributed by @iot-rocket.
- **CloudFormation — the `Tags` property of a queue, topic, table, function, log group and stream reaches the service** — `AWS::SQS::Queue`, `AWS::SNS::Topic`, `AWS::DynamoDB::Table`, `AWS::Lambda::Function`, `AWS::Logs::LogGroup` and `AWS::Kinesis::Stream` stored their tags nowhere, so the service's own list-tags call answered an empty set for a tagged resource. The tags now land where `ListQueueTags`, `ListTagsForResource`, `ListTagsOfResource`, `ListTags`, `ListTagsLogGroup` and `ListTagsForStream` read them; a template change is reconciled on update (dropped keys removed, the rest set) and tags added through the service API stay. Contributed by @iot-rocket.
- **CloudFormation — stack-level tags propagate to the resources** — the `Tags` of `CreateStack` and `UpdateStack` were stored on the stack record and went nowhere, and no resource carried the `aws:cloudformation:stack-name`, `aws:cloudformation:stack-id` and `aws:cloudformation:logical-id` tags a real deploy adds. For the 44 types whose provisioner stores a tag property (list-shaped `Tags`, the map-shaped `Tags` of SSM and API Gateway v2, `UserPoolTags`, `IdentityPoolTags`, `BackupVaultTags`, `BackupPlanTags`), the stack tags and the three `aws:cloudformation:` tags are merged into that property before the resource is created or updated; a key the template sets wins. A stack-tag change on `UpdateStack` reaches the resources whose type has an update handler (the 20 types without one keep their tags and are not re-created), a nested stack passes the tags on, an empty `Tags` on `UpdateStack` or a change set removes the stack's tags (as the API documents for an empty value), more than 50 tags or an `aws:` prefixed key are refused before a stack exists, and the stack record keeps the template's own properties, so change sets and the no-change check still compare templates. Contributed by @iot-rocket.
- **CloudFormation — the list and describe actions page their results** — `DescribeStacks`, `ListStacks`, `DescribeStackEvents`, `ListStackResources`, `ListExports`, `ListImports` and `ListChangeSets` answered the whole store in one response and ignored `NextToken`, so a paginator drained nothing and a caller that pages (`ListExports` pages at 100 values on AWS, the others at 1 MB) never saw a second page. They now return 100 items per page with a `NextToken` for the rest; a token the service did not hand out is a `ValidationError`. `DescribeStackEvents` also orders events of one millisecond newest first, so the pages are strictly reverse-chronological. Contributed by @iot-rocket.
- **CloudFormation — `DescribeStackResource` returns the resource `Metadata`** — the `Metadata` attribute of a resource was accepted and never returned; `StackResourceDetail` now carries it as a JSON string with intrinsics interpreted, as the API reference defines it (a CDK stack's `aws:cdk:path` is the common case); a `Metadata` that resolves away or cannot be resolved comes back as declared. Contributed by @iot-rocket.
### Fixed
...
- **CloudFormation — the stack id addresses `GetTemplateSummary` and an UPDATE change set, and the Describe*Resource responses carry the stack name** — `GetTemplateSummary(StackName=<id>)` and `CreateChangeSet` on an existing stack looked the id up by name and answered "does not exist" while every other action accepted it; `DescribeStackResource` and `DescribeStackResources` answered a request by id with the id in `StackName`. Contributed by @iot-rocket.
### Added
