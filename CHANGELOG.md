...
### Added
- **ALB — the `authenticate-oidc` listener action authenticates** — a rule accepted the action's `Type` and silently discarded its `AuthenticateOidcConfig`, so an OIDC-protected service tested against MiniStack passed while every request reached the target unauthenticated. The flow now runs end to end as AWS documents it: an unauthenticated request is redirected to the `AuthorizationEndpoint` with `state`, the callback at `/oauth2/idpresponse` exchanges the code at the `TokenEndpoint` and fetches the claims from the `UserInfoEndpoint`, and the session is written to the client as `AWSELBAuthSessionCookie` shards sized so the whole Set-Cookie pair fits the 4096-byte browser ceiling. Later requests reassemble and validate the shards and reach the target with `x-amzn-oidc-identity`, `x-amzn-oidc-accesstoken` and `x-amzn-oidc-data` — the last as the user claims in AWS's signed-JWT shape (`kid`, `signer`, `iss`, `client`, `exp` in the header; the provider's ID token is never passed to the backend), with any client-supplied copies of those headers stripped so an identity cannot be forged even under `OnUnauthenticatedRequest=allow`. `SessionCookieName`, `SessionTimeout`, `Scope` and all three `OnUnauthenticatedRequest` modes are honoured; an unmatched or expired callback answers `401` per the 15-minute login bound; `ClientSecret` is held for the exchange and never echoed from `Describe*`. Two data-plane fixes came with it: a rule's actions now run as a chain in `Order` (dispatch executed only `Actions[0]`, so authenticate-then-forward could not work), and `ModifyListener` now updates the auto-created default rule the data plane actually serves. `authenticate-cognito` answers `501` rather than forwarding a request nobody authenticated. Contributed by @dhanesh.

- **Router — an unregistered `Host` is no longer routed by service-token substring** — the host-pattern step of `detect_service()` ran the unanchored `iot\.`, `logs\.`, `email\.`, `lambda\.` … regexes over any `Host` the earlier steps (X-Amz-Target, SigV4 credential scope, query `Action`, path) had not classified, so a request for `probe.iot.example.com` answered `Unsupported IoT path` and `logs.example.com` landed in CloudWatch Logs — a customer domain behind a proxy, or a health check with a made-up `Host`, was read as an AWS endpoint. Host patterns are now consulted only for hosts the stack serves: a single label (`ministack`, `localhost`, a compose service name), a two-label alias (`s3.dev`), an IP literal, or a name under `localhost`, `localhost.localstack.cloud`, `amazonaws.com`, `MINISTACK_HOST`, the container hostname or the new comma-separated `MINISTACK_EXTRA_HOST_SUFFIXES`, matched at a label boundary. Every other `Host` falls through to the default as before. On a served host each bare service token is anchored at a label start (`(^|\.)iot\.`), so `probe-iot.localhost` is not IoT either; `<bucket>.s3.<region>`, `<api-id>.execute-api.<region>`, `<prefix>-ats.iot.<region>`, `streams.dynamodb.` and the other AWS shapes route exactly as they did. Contributed by @iot-rocket.
...
- **Amazon Transcribe (`transcribe`)** is a new service emulator covering batch transcription jobs: `StartTranscriptionJob`, `GetTranscriptionJob`, `ListTranscriptionJobs` and `DeleteTranscriptionJob`, verified against botocore `transcribe-2017-10-26`. A job walks `QUEUED` → `IN_PROGRESS` → `COMPLETED` on a background task paced by `TRANSCRIBE_JOB_RUN_SECONDS` (default 2, `0` completes immediately), reads its media object from MiniStack's S3 store through an `s3://`, path-style or virtual-hosted HTTPS URI, and writes a transcript document back to S3 in the real Transcribe result format: `results.transcripts`, word-level `results.items`, `results.audio_segments`, and `results.segments` / `results.speaker_labels` when `ShowAlternatives` and `ShowSpeakerLabels` are set. `IdentifyLanguage` fills in `LanguageCode` and `IdentifiedLanguageScore` on completion while `IdentifyMultipleLanguages` reports `LanguageCodes` and leaves `LanguageCode` unset, as AWS does. `ContentRedaction` decides whether the `redacted-` copy replaces the transcript or sits beside it, and `Subtitles` writes WebVTT and SubRip files reported as `SubtitleFileUris`, redacted and renamed to match when redaction is on. Output lands at `OutputBucketName` / `OutputKey`, or in an auto-created `ministack-transcribe-output` bucket when neither is given, so the reported URIs are always fetchable. Media that cannot be read fails the job asynchronously with a `FailureReason` rather than erroring at `Start`, matching AWS, and a job left mid-flight by a restart fails on restore instead of stranding pollers. Reaching a terminal state publishes a `Transcribe Job State Change` event on the default bus, so an S3 → Lambda → Transcribe → EventBridge pipeline runs end to end locally. There is **no speech recognition**: the transcript is deterministic canned text keyed on a digest of the media URI, the same approach Bedrock Runtime takes for model output, which keeps integration tests reproducible. Streaming transcription, the call-analytics and medical job families, custom vocabularies and the tagging operations are not implemented. State is account- and region-scoped and persisted. Contributed by @ppettitau.

### Fixed
- **Lambda — a failed Docker cold start no longer leaks its extraction directory** — the executor unpacks the function zip and its layers into a `ministack-lambda-docker-*` temp directory before talking to Docker, but only three specific failure paths cleaned it up; anything else raised between the extraction and the container start — a corrupt layer zip, or a Docker socket read-timeout out of the image lookup (which is not `ImageNotFound`) — orphaned the directory. Under Docker-API pressure that compounds: each timed-out spawn leaves a full unpacked tree behind, measured at gigabytes of `/tmp` in minutes on a large stack. Cleanup now covers every exit: the directory is owned by the spawn until the container is handed to the warm pool, and removed on any failure in between. Reported by @iot-rocket.
