...
- **Lambda (Docker executor) — container callbacks resolve on native Linux engines** — `AWS_ENDPOINT_URL` inside a Lambda container is rewritten to `host.docker.internal`, which only resolves on Docker Desktop; on a Linux engine every nested SDK call a handler made died on DNS (`<urlopen error [Errno 16] Device or resource busy>`). Containers now map that name to `host-gateway`, as the ECS and EKS paths already do; an explicit `--add-host` still wins. Contributed by @iot-rocket.
